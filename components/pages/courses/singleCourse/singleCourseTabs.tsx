// components/pages/courses/singleCourse/singleCourseTabs.tsx
'use client';
...
export function SingleCourseTabs({
  ...
  return (
    <Tabs className="justify-center items-center mx-auto w-full" value={activeTab} onValueChange={setActiveTab}>
      <div className="relative w-full">
        {/* Tabs for Desktop/Tablet */}
        <div className="hidden sm:block">
          <ScrollArea className="w-full">
            <TabsList className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-2 p-1 bg-gray-100 rounded-lg">
        ...
        <div className="sm:hidden">
          <TabsList className="grid grid-cols-3 gap-2 p-1 bg-gray-100 rounded-lg">
            <TabsTrigger value="overview">Overview</TabsTrigger>
            <TabsTrigger value="curriculum">Curriculum</TabsTrigger>
            <TabsTrigger value="instructor">Instructor</TabsTrigger>
          </TabsList>
          <MobileTabs onTabChange={setActiveTab} activeTab={activeTab} />
        </div>
      </div>
      <TabsContent value="overview">
        <div className="text-wrap">
          <h2 className="text-lg md:text-xl lg:text-2xl font-bold">Course Description</h2>
          <p className="mt-4 text-gray-700">{courseDetails.description}</p>
          <h3 className="mt-6 text-base md:text-lg lg:text-xl font-semibold">What You'll Learn?</h3>
          <ul className="list-disc pl-6 mt-4 space-y-2 text-gray-600">
            {courseDetails.summary.split("---").map((summaryList: string, i: number) => (
              <li key={i}>{summaryList}</li>
            ))}
          </ul>
          <p className="mt-4 text-gray-700">{courseDetails.conclusion}</p>
        </div>
      </TabsContent>
      <TabsContent value="curriculum">
        {user && isPaid ? (
          <SingleCourseCurriculum modules={modules} progress={progress} />
        ) : !isPaid ? (
      ...
      <TabsContent value="instructor">
        {user && isPaid ? (
          <TutorProfile tutorName={tutor.name ?? "Tutor"} highestAverageRating={reviews?.reduce((total: number, review: any) => total + review.rating, 0) ?? 0} />
        ) : !isPaid ? (
          <div className="mx-auto items-center justify-center text-center">
            <p className="py-10 text-base">Enroll for this course to get complete access!</p>
            <Button asChild>
      ...
      <TabsContent value="reviews">
        {user && isPaid ? (
          <SingleTutorReviews reviews={reviews ?? []} tutor={tutor} courseId={courseDetails.id} />
        ) : !isPaid ? (
          <div className="mx-auto items-center justify-center text-center">
            <p className="py-10 text-base">Enroll for this course to get complete access!</p>
            <Button asChild>
