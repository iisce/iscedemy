import { SVGProps } from "react";

export const SearchIcon = (
     <svg
          width="20"
          height="20"
          viewBox="0 0 70 70"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
     >
          <path
               d="M61.25 61.25L51.0417 51.0417M49.5833 29.1667C49.5833 31.8478 49.0552 34.5027 48.0292 36.9798C47.0032 39.4569 45.4993 41.7076 43.6034 43.6034C41.7076 45.4993 39.4569 47.0032 36.9798 48.0292C34.5027 49.0552 31.8478 49.5833 29.1667 49.5833C26.4855 49.5833 23.8306 49.0552 21.3535 48.0292C18.8765 47.0032 16.6258 45.4993 14.7299 43.6034C12.834 41.7076 11.3302 39.4569 10.3041 36.9798C9.27809 34.5027 8.75 31.8478 8.75 29.1667C8.75 23.7518 10.901 18.5588 14.7299 14.7299C18.5588 10.901 23.7518 8.75 29.1667 8.75C34.5815 8.75 39.7746 10.901 43.6034 14.7299C47.4323 18.5588 49.5833 23.7518 49.5833 29.1667Z"
               stroke="#171717"
               strokeWidth="5.83333"
               strokeLinecap="round"
          />
     </svg>
);

export const ArrowRightIcon = () => (
     <svg
          width="30"
          height="30"
          viewBox="0 0 70 70"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
     >
          <path
               d="M52.5 35H14.5833"
               stroke="#ffffff"
               strokeWidth="5.83333"
               strokeLinecap="round"
          />
          <path
               d="M43.75 46.6667L55.4167 35L43.75 23.3333"
               stroke="#ffffff"
               strokeWidth="5.83333"
               strokeLinecap="round"
               strokeLinejoin="round"
          />
     </svg>
);

export const CircleRightArrow = (
     <svg
          width="16"
          height="16"
          viewBox="0 0 70 70"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
     >
          <path
               d="M35.1516 43.2512L39.2758 47.3754L51.6541 35L39.2787 22.6246L35.1546 26.7517L40.4833 32.0833H18.5004V37.9167H40.4833L35.1516 43.2512Z"
               fill="#171717"
          />
          <path
               fillRule="evenodd"
               clipRule="evenodd"
               d="M57.6858 57.6858C70.2158 45.1558 70.2158 24.8442 57.6858 12.3142C45.1558 -0.215836 24.8442 -0.215836 12.3142 12.3142C-0.215844 24.8442 -0.215844 45.1558 12.3142 57.6858C24.8442 70.2158 45.1558 70.2158 57.6858 57.6858ZM53.5617 53.5617C55.9992 51.1241 57.9328 48.2303 59.252 45.0455C60.5712 41.8607 61.2502 38.4472 61.2502 35C61.2502 31.5528 60.5712 28.1393 59.252 24.9545C57.9328 21.7697 55.9992 18.8759 53.5617 16.4383C51.1241 14.0008 48.2303 12.0672 45.0455 10.748C41.8607 9.42882 38.4472 8.74984 35 8.74984C31.5528 8.74984 28.1393 9.42882 24.9545 10.748C21.7697 12.0672 18.8759 14.0008 16.4383 16.4383C11.5155 21.3612 8.74983 28.038 8.74983 35C8.74983 41.962 11.5155 48.6388 16.4383 53.5617C21.3612 58.4845 28.038 61.2502 35 61.2502C41.962 61.2502 48.6388 58.4845 53.5617 53.5617Z"
               fill="#171717"
          />
     </svg>
);
export function NodeJSIcon() {
     return (
          <svg
               xmlns="http://www.w3.org/2000/svg"
               width="1.5em"
               height="1.5em"
               viewBox="0 0 24 24"
          >
               <path
                    fill="currentColor"
                    d="M12 1.85c-.27 0-.55.07-.78.2l-7.44 4.3c-.48.28-.78.8-.78 1.36v8.58c0 .56.3 1.08.78 1.36l1.95 1.12c.95.46 1.27.47 1.71.47c1.4 0 2.21-.85 2.21-2.33V8.44c0-.12-.1-.22-.22-.22H8.5c-.13 0-.23.1-.23.22v8.47c0 .66-.68 1.31-1.77.76L4.45 16.5a.26.26 0 0 1-.11-.21V7.71c0-.09.04-.17.11-.21l7.44-4.29c.06-.04.16-.04.22 0l7.44 4.29c.07.04.11.12.11.21v8.58c0 .08-.04.16-.11.21l-7.44 4.29c-.06.04-.16.04-.23 0L10 19.65c-.08-.03-.16-.04-.21-.01c-.53.3-.63.36-1.12.51c-.12.04-.31.11.07.32l2.48 1.47c.24.14.5.21.78.21s.54-.07.78-.21l7.44-4.29c.48-.28.78-.8.78-1.36V7.71c0-.56-.3-1.08-.78-1.36l-7.44-4.3c-.23-.13-.5-.2-.78-.2M14 8c-2.12 0-3.39.89-3.39 2.39c0 1.61 1.26 2.08 3.3 2.28c2.43.24 2.62.6 2.62 1.08c0 .83-.67 1.18-2.23 1.18c-1.98 0-2.4-.49-2.55-1.47a.226.226 0 0 0-.22-.18h-.96c-.12 0-.21.09-.21.22c0 1.24.68 2.74 3.94 2.74c2.35 0 3.7-.93 3.7-2.55c0-1.61-1.08-2.03-3.37-2.34c-2.31-.3-2.54-.46-2.54-1c0-.45.2-1.05 1.91-1.05c1.5 0 2.09.33 2.32 1.36c.02.1.11.17.21.17h.97c.05 0 .11-.02.15-.07c.04-.04.07-.1.05-.16C17.56 8.82 16.38 8 14 8"
               />
          </svg>
     );
}
export function NodeJSCourseIcon() {
     return (
          <svg
               xmlns="http://www.w3.org/2000/svg"
               width="1.5em"
               height="1.5em"
               viewBox="0 0 24 24"
          >
               <path
                    fill="currentColor"
                    d="M12 1.85c-.27 0-.55.07-.78.2l-7.44 4.3c-.48.28-.78.8-.78 1.36v8.58c0 .56.3 1.08.78 1.36l1.95 1.12c.95.46 1.27.47 1.71.47c1.4 0 2.21-.85 2.21-2.33V8.44c0-.12-.1-.22-.22-.22H8.5c-.13 0-.23.1-.23.22v8.47c0 .66-.68 1.31-1.77.76L4.45 16.5a.26.26 0 0 1-.11-.21V7.71c0-.09.04-.17.11-.21l7.44-4.29c.06-.04.16-.04.22 0l7.44 4.29c.07.04.11.12.11.21v8.58c0 .08-.04.16-.11.21l-7.44 4.29c-.06.04-.16.04-.23 0L10 19.65c-.08-.03-.16-.04-.21-.01c-.53.3-.63.36-1.12.51c-.12.04-.31.11.07.32l2.48 1.47c.24.14.5.21.78.21s.54-.07.78-.21l7.44-4.29c.48-.28.78-.8.78-1.36V7.71c0-.56-.3-1.08-.78-1.36l-7.44-4.3c-.23-.13-.5-.2-.78-.2M14 8c-2.12 0-3.39.89-3.39 2.39c0 1.61 1.26 2.08 3.3 2.28c2.43.24 2.62.6 2.62 1.08c0 .83-.67 1.18-2.23 1.18c-1.98 0-2.4-.49-2.55-1.47a.226.226 0 0 0-.22-.18h-.96c-.12 0-.21.09-.21.22c0 1.24.68 2.74 3.94 2.74c2.35 0 3.7-.93 3.7-2.55c0-1.61-1.08-2.03-3.37-2.34c-2.31-.3-2.54-.46-2.54-1c0-.45.2-1.05 1.91-1.05c1.5 0 2.09.33 2.32 1.36c.02.1.11.17.21.17h.97c.05 0 .11-.02.15-.07c.04-.04.07-.1.05-.16C17.56 8.82 16.38 8 14 8"
               />
          </svg>
     );
}
export function HtmlIcon() {
     return (
          <svg
               xmlns="http://www.w3.org/2000/svg"
               width="1em"
               height="1em"
               viewBox="0 0 16 16"
          >
               <path
                    fill="currentColor"
                    d="M.946 0L2.23 14.4L7.992 16l5.777-1.602L15.055 0zm11.722 13.482l-4.644 1.287v.007l-.012-.004l-.012.004v-.007l-4.644-1.287L2.258 1.178h11.508zm-2.5-5.198l-.204 2.29l-1.972.532l-1.967-.53l-.126-1.41H4.126l.247 2.774l3.626 1.003l3.615-1.003l.485-5.422H5.662l-.161-1.809h6.758l.158-1.766H3.57l.477 5.341z"
               />
          </svg>
     );
}
export function JavaScriptIcon() {
     return (
          <svg
               xmlns="http://www.w3.org/2000/svg"
               width="1.5em"
               height="1.5em"
               viewBox="0 0 24 24"
          >
               <path
                    fill="currentColor"
                    d="M6 3a3 3 0 0 0-3 3v12a3 3 0 0 0 3 3h12a3 3 0 0 0 3-3V6a3 3 0 0 0-3-3zm7.334 13.055c.72.58 1.438.865 2.156.858c.44 0 .778-.08 1.012-.242a.75.75 0 0 0 .341-.66a.971.971 0 0 0-.34-.748c-.235-.205-.679-.41-1.332-.616c-.784-.227-1.39-.52-1.815-.88c-.418-.36-.63-.862-.638-1.507c0-.609.264-1.118.792-1.529c.514-.41 1.17-.616 1.97-.616c1.114 0 2.009.271 2.683.814l-.77 1.199a2.597 2.597 0 0 0-.935-.462a3.211 3.211 0 0 0-.946-.165c-.38 0-.685.07-.913.209c-.227.14-.34.323-.34.55c0 .25.139.462.417.638c.28.169.756.356 1.43.561c.814.242 1.394.565 1.738.968c.345.403.517.917.517 1.54c0 .638-.245 1.188-.737 1.65c-.484.455-1.188.693-2.112.715c-1.21 0-2.222-.363-3.036-1.089zm-5.53.638c.235.147.517.22.847.22c.345 0 .63-.099.858-.297c.227-.205.341-.561.341-1.067v-5.302h1.485v5.588c-.022.865-.271 1.489-.748 1.87a2.466 2.466 0 0 1-.891.484a3.296 3.296 0 0 1-.935.143c-.55 0-1.038-.095-1.463-.286c-.455-.205-.836-.568-1.144-1.089l1.034-.847c.19.257.396.451.616.583"
               />
          </svg>
     );
}
export function SqlIcon() {
     return (
          <svg
               xmlns="http://www.w3.org/2000/svg"
               width="1em"
               height="1em"
               viewBox="0 0 24 24"
          >
               <path
                    fill="none"
                    stroke="currentColor"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    strokeWidth="2"
                    d="M12 8a2 2 0 0 1 2 2v4a2 2 0 1 1-4 0v-4a2 2 0 0 1 2-2m5 0v8h4m-8-1l1 1M3 15a1 1 0 0 0 1 1h2a1 1 0 0 0 1-1v-2a1 1 0 0 0-1-1H4a1 1 0 0 1-1-1V9a1 1 0 0 1 1-1h2a1 1 0 0 1 1 1"
               />
          </svg>
     );
}
export function NextJSIcon() {
     return (
          <svg
               xmlns="http://www.w3.org/2000/svg"
               width="1.5em"
               height="1.5em"
               viewBox="0 0 15 15"
          >
               <path
                    fill="currentColor"
                    d="m4.5 4.5l.405-.293A.5.5 0 0 0 4 4.5zm3 9.5A6.5 6.5 0 0 1 1 7.5H0A7.5 7.5 0 0 0 7.5 15zM14 7.5A6.5 6.5 0 0 1 7.5 14v1A7.5 7.5 0 0 0 15 7.5zM7.5 1A6.5 6.5 0 0 1 14 7.5h1A7.5 7.5 0 0 0 7.5 0zm0-1A7.5 7.5 0 0 0 0 7.5h1A6.5 6.5 0 0 1 7.5 1zM5 12V4.5H4V12zm-.905-7.207l6.5 9l.81-.586l-6.5-9zM10 4v6h1V4z"
               />
          </svg>
     );
}
export function ReactJSIcon() {
     return (
          <svg
               xmlns="http://www.w3.org/2000/svg"
               width="1.5em"
               height="1.5em"
               viewBox="0 0 24 24"
          >
               <path
                    fill="currentColor"
                    d="M12 10.11c1.03 0 1.87.84 1.87 1.89c0 1-.84 1.85-1.87 1.85c-1.03 0-1.87-.85-1.87-1.85c0-1.05.84-1.89 1.87-1.89M7.37 20c.63.38 2.01-.2 3.6-1.7c-.52-.59-1.03-1.23-1.51-1.9a22.7 22.7 0 0 1-2.4-.36c-.51 2.14-.32 3.61.31 3.96m.71-5.74l-.29-.51c-.11.29-.22.58-.29.86c.27.06.57.11.88.16zm6.54-.76l.81-1.5l-.81-1.5c-.3-.53-.62-1-.91-1.47C13.17 9 12.6 9 12 9c-.6 0-1.17 0-1.71.03c-.29.47-.61.94-.91 1.47L8.57 12l.81 1.5c.3.53.62 1 .91 1.47c.54.03 1.11.03 1.71.03c.6 0 1.17 0 1.71-.03c.29-.47.61-.94.91-1.47M12 6.78c-.19.22-.39.45-.59.72h1.18c-.2-.27-.4-.5-.59-.72m0 10.44c.19-.22.39-.45.59-.72h-1.18c.2.27.4.5.59.72M16.62 4c-.62-.38-2 .2-3.59 1.7c.52.59 1.03 1.23 1.51 1.9c.82.08 1.63.2 2.4.36c.51-2.14.32-3.61-.32-3.96m-.7 5.74l.29.51c.11-.29.22-.58.29-.86c-.27-.06-.57-.11-.88-.16zm1.45-7.05c1.47.84 1.63 3.05 1.01 5.63c2.54.75 4.37 1.99 4.37 3.68c0 1.69-1.83 2.93-4.37 3.68c.62 2.58.46 4.79-1.01 5.63c-1.46.84-3.45-.12-5.37-1.95c-1.92 1.83-3.91 2.79-5.38 1.95c-1.46-.84-1.62-3.05-1-5.63c-2.54-.75-4.37-1.99-4.37-3.68c0-1.69 1.83-2.93 4.37-3.68c-.62-2.58-.46-4.79 1-5.63c1.47-.84 3.46.12 5.38 1.95c1.92-1.83 3.91-2.79 5.37-1.95M17.08 12c.34.75.64 1.5.89 2.26c2.1-.63 3.28-1.53 3.28-2.26c0-.73-1.18-1.63-3.28-2.26c-.25.76-.55 1.51-.89 2.26M6.92 12c-.34-.75-.64-1.5-.89-2.26c-2.1.63-3.28 1.53-3.28 2.26c0 .73 1.18 1.63 3.28 2.26c.25-.76.55-1.51.89-2.26m9 2.26l-.3.51c.31-.05.61-.1.88-.16c-.07-.28-.18-.57-.29-.86zm-2.89 4.04c1.59 1.5 2.97 2.08 3.59 1.7c.64-.35.83-1.82.32-3.96c-.77.16-1.58.28-2.4.36c-.48.67-.99 1.31-1.51 1.9M8.08 9.74l.3-.51c-.31.05-.61.1-.88.16c.07.28.18.57.29.86zm2.89-4.04C9.38 4.2 8 3.62 7.37 4c-.63.35-.82 1.82-.31 3.96a22.7 22.7 0 0 1 2.4-.36c.48-.67.99-1.31 1.51-1.9"
               />
          </svg>
     );
}
export function BookIcon() {
     return (
          <svg
               xmlns="http://www.w3.org/2000/svg"
               width="24"
               height="24"
               viewBox="0 0 24 24"
               fill="none"
               stroke="currentColor"
               strokeWidth="2"
               strokeLinecap="round"
               strokeLinejoin="round"
          >
               <path d="M4 19.5v-15A2.5 2.5 0 0 1 6.5 2H20v20H6.5a2.5 2.5 0 0 1 0-5H20" />
          </svg>
     );
}
export function CatIcon() {
     return (
          <svg
               xmlns="http://www.w3.org/2000/svg"
               width="24"
               height="24"
               viewBox="0 0 24 24"
               fill="none"
               stroke="currentColor"
               strokeWidth="2"
               strokeLinecap="round"
               strokeLinejoin="round"
          >
               <path d="M12 5c.67 0 1.35.09 2 .26 1.78-2 5.03-2.84 6.42-2.26 1.4.58-.42 7-.42 7 .57 1.07 1 2.24 1 3.44C21 17.9 16.97 21 12 21s-9-3-9-7.56c0-1.25.5-2.4 1-3.44 0 0-1.89-6.42-.5-7 1.39-.58 4.72.23 6.5 2.23A9.04 9.04 0 0 1 12 5Z" />
               <path d="M8 14v.5" />
               <path d="M16 14v.5" />
               <path d="M11.25 16.25h1.5L12 17l-.75-.75Z" />
          </svg>
     );
}
export function DatabaseIcon() {
     return (
          <svg
               xmlns="http://www.w3.org/2000/svg"
               width="24"
               height="24"
               viewBox="0 0 24 24"
               fill="none"
               stroke="currentColor"
               strokeWidth="2"
               strokeLinecap="round"
               strokeLinejoin="round"
          >
               <ellipse cx="12" cy="5" rx="9" ry="3" />
               <path d="M3 5V19A9 3 0 0 0 21 19V5" />
               <path d="M3 12A9 3 0 0 0 21 12" />
          </svg>
     );
}
export function FileCodeIcon() {
     return (
          <svg
               xmlns="http://www.w3.org/2000/svg"
               width="24"
               height="24"
               viewBox="0 0 24 24"
               fill="none"
               stroke="currentColor"
               strokeWidth="2"
               strokeLinecap="round"
               strokeLinejoin="round"
          >
               <path d="M14.5 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7.5L14.5 2z" />
               <polyline points="14 2 14 8 20 8" />
               <path d="m10 13-2 2 2 2" />
               <path d="m14 17 2-2-2-2" />
          </svg>
     );
}
export function Heading5Icon() {
     return (
          <svg
               xmlns="http://www.w3.org/2000/svg"
               width="24"
               height="24"
               viewBox="0 0 24 24"
               fill="none"
               stroke="currentColor"
               strokeWidth="2"
               strokeLinecap="round"
               strokeLinejoin="round"
          >
               <path d="M4 12h8" />
               <path d="M4 18V6" />
               <path d="M12 18V6" />
               <path d="M17 13v-3h4" />
               <path d="M17 17.7c.4.2.8.3 1.3.3 1.5 0 2.7-1.1 2.7-2.5S19.8 13 18.3 13H17" />
          </svg>
     );
}
export function LaptopIcon() {
     return (
          <svg
               xmlns="http://www.w3.org/2000/svg"
               width="24"
               height="24"
               viewBox="0 0 24 24"
               fill="none"
               stroke="currentColor"
               strokeWidth="2"
               strokeLinecap="round"
               strokeLinejoin="round"
          >
               <path d="M20 16V7a2 2 0 0 0-2-2H6a2 2 0 0 0-2 2v9m16 0H4m16 0 1.28 2.55a1 1 0 0 1-.9 1.45H3.62a1 1 0 0 1-.9-1.45L4 16" />
          </svg>
     );
}
export function PaintbrushIcon() {
     return (
          <svg
               xmlns="http://www.w3.org/2000/svg"
               width="24"
               height="24"
               viewBox="0 0 24 24"
               fill="none"
               stroke="currentColor"
               strokeWidth="2"
               strokeLinecap="round"
               strokeLinejoin="round"
          >
               <path d="M18.37 2.63 14 7l-1.59-1.59a2 2 0 0 0-2.82 0L8 7l9 9 1.59-1.59a2 2 0 0 0 0-2.82L17 10l4.37-4.37a2.12 2.12 0 1 0-3-3Z" />
               <path d="M9 8c-2 3-4 3.5-7 4l8 10c2-1 6-5 6-7" />
               <path d="M14.5 17.5 4.5 15" />
          </svg>
     );
}

export function AwardIcon() {
     return (
          <svg
               xmlns="http://www.w3.org/2000/svg"
               width="3em"
               height="3em"
               viewBox="0 0 24 24"
               fill="none"
               stroke="currentColor"
               strokeWidth="2"
               strokeLinecap="round"
               strokeLinejoin="round"
          >
               <circle cx="12" cy="8" r="6" />
               <path d="M15.477 12.89 17 22l-5-3-5 3 1.523-9.11" />
          </svg>
     );
}
export function ExpertIcon() {
     return (
          <svg
               xmlns="http://www.w3.org/2000/svg"
               width="3em"
               height="3em"
               viewBox="0 0 24 24"
          >
               <path
                    fill="none"
                    stroke="currentColor"
                    strokeWidth="2"
                    d="M8 11A5 5 0 1 0 8 1a5 5 0 0 0 0 10Zm6.643 4.696a6.745 6.745 0 0 0-1.62-2.673C11.772 11.76 10.013 11 8 11c-4 0-7 3-7 7v5h10m1-4.176L16.19 22L23 13"
               />
          </svg>
     );
}
export function ProjectIcon() {
     return (
          <svg
               xmlns="http://www.w3.org/2000/svg"
               width="3em"
               height="3em"
               viewBox="0 0 48 48"
          >
               <path
                    fill="none"
                    stroke="currentColor"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    d="M42.79 12.72L21.55 41.78L3.79 28.79l4.87-6.66l11.1 8.12l16.37-22.4l3.312 2.45l.342.223l-19.167 25.533l-11.687-8.43"
               />
          </svg>
     );
}
export function Curriculum() {
     return (
          <svg
               xmlns="http://www.w3.org/2000/svg"
               width="3em"
               height="3em"
               viewBox="0 0 14 14"
          >
               <g fill="none" stroke="currentColor" strokeLinecap="round">
                    <path
                         strokeLinejoin="round"
                         d="M4.098 3.65a1.15 1.15 0 1 0 2.3 0a1.15 1.15 0 0 0-2.3 0"
                    />
                    <path
                         strokeLinejoin="round"
                         d="M3.5 5.94a2.22 2.22 0 0 1 .746-.834c.306-.2.652-.304 1.003-.304c.351 0 .697.104 1.003.304c.307.199.563.486.746.834"
                    />
                    <path
                         strokeLinejoin="round"
                         d="M12.5 12.5a1 1 0 0 1-1 1h-9a1 1 0 0 1-1-1v-11a1 1 0 0 1 1-1H9L12.5 4z"
                    />
                    <path d="M3.5 8.5h7m-7 2.5h4" />
               </g>
          </svg>
     );
}
export function ReactJSColorIcon() {
     return (
          <svg
               xmlns="http://www.w3.org/2000/svg"
               width="1.5em"
               height="1.5em"
               viewBox="0 0 256 228"
          >
               <path
                    fill="CurrentColor"
                    d="M210.483 73.824a171.49 171.49 0 0 0-8.24-2.597c.465-1.9.893-3.777 1.273-5.621c6.238-30.281 2.16-54.676-11.769-62.708c-13.355-7.7-35.196.329-57.254 19.526a171.23 171.23 0 0 0-6.375 5.848a155.866 155.866 0 0 0-4.241-3.917C100.759 3.829 77.587-4.822 63.673 3.233C50.33 10.957 46.379 33.89 51.995 62.588a170.974 170.974 0 0 0 1.892 8.48c-3.28.932-6.445 1.924-9.474 2.98C17.309 83.498 0 98.307 0 113.668c0 15.865 18.582 31.778 46.812 41.427a145.52 145.52 0 0 0 6.921 2.165a167.467 167.467 0 0 0-2.01 9.138c-5.354 28.2-1.173 50.591 12.134 58.266c13.744 7.926 36.812-.22 59.273-19.855a145.567 145.567 0 0 0 5.342-4.923a168.064 168.064 0 0 0 6.92 6.314c21.758 18.722 43.246 26.282 56.54 18.586c13.731-7.949 18.194-32.003 12.4-61.268a145.016 145.016 0 0 0-1.535-6.842c1.62-.48 3.21-.974 4.76-1.488c29.348-9.723 48.443-25.443 48.443-41.52c0-15.417-17.868-30.326-45.517-39.844m-6.365 70.984c-1.4.463-2.836.91-4.3 1.345c-3.24-10.257-7.612-21.163-12.963-32.432c5.106-11 9.31-21.767 12.459-31.957c2.619.758 5.16 1.557 7.61 2.4c23.69 8.156 38.14 20.213 38.14 29.504c0 9.896-15.606 22.743-40.946 31.14m-10.514 20.834c2.562 12.94 2.927 24.64 1.23 33.787c-1.524 8.219-4.59 13.698-8.382 15.893c-8.067 4.67-25.32-1.4-43.927-17.412a156.726 156.726 0 0 1-6.437-5.87c7.214-7.889 14.423-17.06 21.459-27.246c12.376-1.098 24.068-2.894 34.671-5.345a134.17 134.17 0 0 1 1.386 6.193M87.276 214.515c-7.882 2.783-14.16 2.863-17.955.675c-8.075-4.657-11.432-22.636-6.853-46.752a156.923 156.923 0 0 1 1.869-8.499c10.486 2.32 22.093 3.988 34.498 4.994c7.084 9.967 14.501 19.128 21.976 27.15a134.668 134.668 0 0 1-4.877 4.492c-9.933 8.682-19.886 14.842-28.658 17.94M50.35 144.747c-12.483-4.267-22.792-9.812-29.858-15.863c-6.35-5.437-9.555-10.836-9.555-15.216c0-9.322 13.897-21.212 37.076-29.293c2.813-.98 5.757-1.905 8.812-2.773c3.204 10.42 7.406 21.315 12.477 32.332c-5.137 11.18-9.399 22.249-12.634 32.792a134.718 134.718 0 0 1-6.318-1.979m12.378-84.26c-4.811-24.587-1.616-43.134 6.425-47.789c8.564-4.958 27.502 2.111 47.463 19.835a144.318 144.318 0 0 1 3.841 3.545c-7.438 7.987-14.787 17.08-21.808 26.988c-12.04 1.116-23.565 2.908-34.161 5.309a160.342 160.342 0 0 1-1.76-7.887m110.427 27.268a347.8 347.8 0 0 0-7.785-12.803c8.168 1.033 15.994 2.404 23.343 4.08c-2.206 7.072-4.956 14.465-8.193 22.045a381.151 381.151 0 0 0-7.365-13.322m-45.032-43.861c5.044 5.465 10.096 11.566 15.065 18.186a322.04 322.04 0 0 0-30.257-.006c4.974-6.559 10.069-12.652 15.192-18.18M82.802 87.83a323.167 323.167 0 0 0-7.227 13.238c-3.184-7.553-5.909-14.98-8.134-22.152c7.304-1.634 15.093-2.97 23.209-3.984a321.524 321.524 0 0 0-7.848 12.897zm8.081 65.352c-8.385-.936-16.291-2.203-23.593-3.793c2.26-7.3 5.045-14.885 8.298-22.6a321.187 321.187 0 0 0 7.257 13.246c2.594 4.48 5.28 8.868 8.038 13.147m37.542 31.03c-5.184-5.592-10.354-11.779-15.403-18.433c4.902.192 9.899.29 14.978.29c5.218 0 10.376-.117 15.453-.343c-4.985 6.774-10.018 12.97-15.028 18.486m52.198-57.817c3.422 7.8 6.306 15.345 8.596 22.52c-7.422 1.694-15.436 3.058-23.88 4.071a382.417 382.417 0 0 0 7.859-13.026a347.403 347.403 0 0 0 7.425-13.565m-16.898 8.101a358.557 358.557 0 0 1-12.281 19.815a329.4 329.4 0 0 1-23.444.823c-7.967 0-15.716-.248-23.178-.732a310.202 310.202 0 0 1-12.513-19.846h.001a307.41 307.41 0 0 1-10.923-20.627a310.278 310.278 0 0 1 10.89-20.637l-.001.001a307.318 307.318 0 0 1 12.413-19.761c7.613-.576 15.42-.876 23.31-.876H128c7.926 0 15.743.303 23.354.883a329.357 329.357 0 0 1 12.335 19.695a358.489 358.489 0 0 1 11.036 20.54a329.472 329.472 0 0 1-11 20.722m22.56-122.124c8.572 4.944 11.906 24.881 6.52 51.026c-.344 1.668-.73 3.367-1.15 5.09c-10.622-2.452-22.155-4.275-34.23-5.408c-7.034-10.017-14.323-19.124-21.64-27.008a160.789 160.789 0 0 1 5.888-5.4c18.9-16.447 36.564-22.941 44.612-18.3M128 90.808c12.625 0 22.86 10.235 22.86 22.86s-10.235 22.86-22.86 22.86s-22.86-10.235-22.86-22.86s10.235-22.86 22.86-22.86"
               />
          </svg>
     );
}
export function GitIcon() {
     return (
          <svg
               xmlns="http://www.w3.org/2000/svg"
               width="1.5em"
               height="1.5em"
               viewBox="0 0 128 128"
          >
               <path
                    fill="CurrentColor"
                    d="M124.737 58.378L69.621 3.264c-3.172-3.174-8.32-3.174-11.497 0L46.68 14.71l14.518 14.518c3.375-1.139 7.243-.375 9.932 2.314c2.703 2.706 3.461 6.607 2.294 9.993l13.992 13.993c3.385-1.167 7.292-.413 9.994 2.295c3.78 3.777 3.78 9.9 0 13.679a9.673 9.673 0 0 1-13.683 0a9.677 9.677 0 0 1-2.105-10.521L68.574 47.933l-.002 34.341a9.708 9.708 0 0 1 2.559 1.828c3.778 3.777 3.778 9.898 0 13.683c-3.779 3.777-9.904 3.777-13.679 0c-3.778-3.784-3.778-9.905 0-13.683a9.65 9.65 0 0 1 3.167-2.11V47.333a9.581 9.581 0 0 1-3.167-2.111c-2.862-2.86-3.551-7.06-2.083-10.576L41.056 20.333L3.264 58.123a8.133 8.133 0 0 0 0 11.5l55.117 55.114c3.174 3.174 8.32 3.174 11.499 0l54.858-54.858a8.135 8.135 0 0 0-.001-11.501"
               />
          </svg>
     );
}
export function DataBaseColorIon() {
     return (
          <svg
               xmlns="http://www.w3.org/2000/svg"
               width="1.5em"
               height="1.5em"
               viewBox="0 0 16 16"
          >
               <path
                    fill="currentColor"
                    d="M4.318 2.687C5.234 2.271 6.536 2 8 2s2.766.27 3.682.687C12.644 3.125 13 3.627 13 4c0 .374-.356.875-1.318 1.313C10.766 5.729 9.464 6 8 6s-2.766-.27-3.682-.687C3.356 4.875 3 4.373 3 4c0-.374.356-.875 1.318-1.313M13 5.698V7c0 .374-.356.875-1.318 1.313C10.766 8.729 9.464 9 8 9s-2.766-.27-3.682-.687C3.356 7.875 3 7.373 3 7V5.698c.271.202.58.378.904.525C4.978 6.711 6.427 7 8 7s3.022-.289 4.096-.777A5 5 0 0 0 13 5.698M14 4c0-1.007-.875-1.755-1.904-2.223C11.022 1.289 9.573 1 8 1s-3.022.289-4.096.777C2.875 2.245 2 2.993 2 4v9c0 1.007.875 1.755 1.904 2.223C4.978 15.71 6.427 16 8 16s3.022-.289 4.096-.777C13.125 14.755 14 14.007 14 13zm-1 4.698V10c0 .374-.356.875-1.318 1.313C10.766 11.729 9.464 12 8 12s-2.766-.27-3.682-.687C3.356 10.875 3 10.373 3 10V8.698c.271.202.58.378.904.525C4.978 9.71 6.427 10 8 10s3.022-.289 4.096-.777A5 5 0 0 0 13 8.698m0 3V13c0 .374-.356.875-1.318 1.313C10.766 14.729 9.464 15 8 15s-2.766-.27-3.682-.687C3.356 13.875 3 13.373 3 13v-1.302c.271.202.58.378.904.525C4.978 12.71 6.427 13 8 13s3.022-.289 4.096-.777c.324-.147.633-.323.904-.525"
               />
          </svg>
     );
}
export function TrendIcon() {
     return (
          <svg
               xmlns="http://www.w3.org/2000/svg"
               width="1.5em"
               height="1.5em"
               viewBox="0 0 48 48"
          >
               <path
                    fill="currentColor"
                    d="M8.5 7.25a1.25 1.25 0 1 0-2.5 0v28.5A6.25 6.25 0 0 0 12.25 42h28.5a1.25 1.25 0 1 0 0-2.5h-28.5a3.75 3.75 0 0 1-3.75-3.75zm19.5 6c0-.69.56-1.25 1.25-1.25h10c.69 0 1.25.56 1.25 1.25v10a1.25 1.25 0 1 1-2.5 0v-6.982L27.884 26.384a1.25 1.25 0 0 1-1.725.04l-4.618-4.197l-7.407 7.407a1.25 1.25 0 0 1-1.768-1.768l8.25-8.25a1.25 1.25 0 0 1 1.725-.04l4.618 4.197l9.273-9.273H29.25c-.69 0-1.25-.56-1.25-1.25"
               />
          </svg>
     );
}
export const BadgeCheckIcon = () => {
     return (
          <svg
               xmlns="http://www.w3.org/2000/svg"
               width="24"
               height="24"
               viewBox="0 0 24 24"
               fill="none"
               stroke="currentColor"
               strokeWidth="2"
               strokeLinecap="round"
               strokeLinejoin="round"
          >
               <path d="M3.85 8.62a4 4 0 0 1 4.78-4.77 4 4 0 0 1 6.74 0 4 4 0 0 1 4.78 4.78 4 4 0 0 1 0 6.74 4 4 0 0 1-4.77 4.78 4 4 0 0 1-6.75 0 4 4 0 0 1-4.78-4.77 4 4 0 0 1 0-6.76Z" />
               <path d="m9 12 2 2 4-4" />
          </svg>
     );
};

export const BookOpenIcon = () => {
     return (
          <svg
               xmlns="http://www.w3.org/2000/svg"
               width="24"
               height="24"
               viewBox="0 0 24 24"
               fill="none"
               stroke="currentColor"
               strokeWidth="2"
               strokeLinecap="round"
               strokeLinejoin="round"
          >
               <path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z" />
               <path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z" />
          </svg>
     );
};

export const ClockIcon = () => {
     return (
          <svg
               xmlns="http://www.w3.org/2000/svg"
               width="24"
               height="24"
               viewBox="0 0 24 24"
               fill="none"
               stroke="currentColor"
               strokeWidth="2"
               strokeLinecap="round"
               strokeLinejoin="round"
          >
               <circle cx="12" cy="12" r="10" />
               <polyline points="12 6 12 12 16 14" />
          </svg>
     );
};

export const CurrencyIcon = () => {
     return (
          <svg
               xmlns="http://www.w3.org/2000/svg"
               width="24"
               height="24"
               viewBox="0 0 24 24"
               fill="none"
               stroke="currentColor"
               strokeWidth="2"
               strokeLinecap="round"
               strokeLinejoin="round"
          >
               <circle cx="12" cy="12" r="8" />
               <line x1="3" x2="6" y1="3" y2="6" />
               <line x1="21" x2="18" y1="3" y2="6" />
               <line x1="3" x2="6" y1="21" y2="18" />
               <line x1="21" x2="18" y1="21" y2="18" />
          </svg>
     );
};

export const FacebookIcon = () => {
     return (
          <svg
               xmlns="http://www.w3.org/2000/svg"
               width="24"
               height="24"
               viewBox="0 0 24 24"
               fill="none"
               stroke="currentColor"
               strokeWidth="2"
               strokeLinecap="round"
               strokeLinejoin="round"
          >
               <path d="M18 2h-3a5 5 0 0 0-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 0 1 1-1h3z" />
          </svg>
     );
};

export const GlobeIcon = () => {
     return (
          <svg
               xmlns="http://www.w3.org/2000/svg"
               width="24"
               height="24"
               viewBox="0 0 24 24"
               fill="none"
               stroke="currentColor"
               strokeWidth="2"
               strokeLinecap="round"
               strokeLinejoin="round"
          >
               <circle cx="12" cy="12" r="10" />
               <line x1="2" x2="22" y1="12" y2="12" />
               <path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z" />
          </svg>
     );
};

export const LinkedinIcon = () => {
     return (
          <svg
               xmlns="http://www.w3.org/2000/svg"
               width="24"
               height="24"
               viewBox="0 0 24 24"
               fill="none"
               stroke="currentColor"
               strokeWidth="2"
               strokeLinecap="round"
               strokeLinejoin="round"
          >
               <path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z" />
               <rect width="4" height="12" x="2" y="9" />
               <circle cx="4" cy="4" r="2" />
          </svg>
     );
};

export const UserIcon = () => {
     return (
          <svg
               xmlns="http://www.w3.org/2000/svg"
               width="24"
               height="24"
               viewBox="0 0 24 24"
               fill="none"
               stroke="currentColor"
               strokeWidth="2"
               strokeLinecap="round"
               strokeLinejoin="round"
          >
               <path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2" />
               <circle cx="12" cy="7" r="4" />
          </svg>
     );
};

export const StarIcon = ({ className, ...props }: SVGProps<SVGSVGElement>) => {
     return (
          <svg
               xmlns="http://www.w3.org/2000/svg"
               width="24"
               height="24"
               viewBox="0 0 24 24"
               fill="none"
               stroke="currentColor"
               strokeWidth="2"
               strokeLinecap="round"
               strokeLinejoin="round"
               className={className}
               {...props}
          >
               <polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" />
          </svg>
     );
};

export const TwitterIcon = () => {
     return (
          <svg
               xmlns="http://www.w3.org/2000/svg"
               width="24"
               height="24"
               viewBox="0 0 24 24"
               fill="none"
               stroke="currentColor"
               strokeWidth="2"
               strokeLinecap="round"
               strokeLinejoin="round"
          >
               <path d="M22 4s-.7 2.1-2 3.4c1.6 10-9.4 17.3-18 11.6 2.2.1 4.4-.6 6-2C3 15.5.5 9.6 3 5c2.2 2.6 5.6 4.1 9 4-.9-4.2 4-6.6 7-3.8 1.1 0 3-1.2 3-1.2z" />
          </svg>
     );
};

export const UsersIcon = () => {
     return (
          <svg
               xmlns="http://www.w3.org/2000/svg"
               width="24"
               height="24"
               viewBox="0 0 24 24"
               fill="none"
               stroke="currentColor"
               strokeWidth="2"
               strokeLinecap="round"
               strokeLinejoin="round"
          >
               <path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2" />
               <circle cx="9" cy="7" r="4" />
               <path d="M22 21v-2a4 4 0 0 0-3-3.87" />
               <path d="M16 3.13a4 4 0 0 1 0 7.75" />
          </svg>
     );
};

export const NairaSignIcon = () => {
     return (
          <svg
               xmlns="http://www.w3.org/2000/svg"
               viewBox="0 0 448 512"
               width="24"
               height="24"
               fill="currentColor"
               stroke="currentColor"
               strokeWidth="2"
               strokeLinecap="round"
               strokeLinejoin="round"
          >
               <path d="M122.6 46.3c-7.8-11.7-22.4-17-35.9-12.9S64 49.9 64 64V256H32c-17.7 0-32 14.3-32 32s14.3 32 32 32H64V448c0 17.7 14.3 32 32 32s32-14.3 32-32V320H228.2l97.2 145.8c7.8 11.7 22.4 17 35.9 12.9s22.7-16.5 22.7-30.6V320h32c17.7 0 32-14.3 32-32s-14.3-32-32-32H384V64c0-17.7-14.3-32-32-32s-32 14.3-32 32V256H262.5L122.6 46.3zM305.1 320H320v22.3L305.1 320zM185.5 256H128V169.7L185.5 256z" />
          </svg>
     );
};
export const FileIcon = () => {
     return (
          <svg
               xmlns="http://www.w3.org/2000/svg"
               width="24"
               height="24"
               viewBox="0 0 24 24"
               fill="none"
               stroke="currentColor"
               strokeWidth="2"
               strokeLinecap="round"
               strokeLinejoin="round"
          >
               <path d="M14.5 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7.5L14.5 2z" />
               <polyline points="14 2 14 8 20 8" />
          </svg>
     );
};
export const StarReviewIcon = () => {
     return (
          <svg
               xmlns="http://www.w3.org/2000/svg"
               width="24"
               height="24"
               viewBox="0 0 24 24"
               fill="none"
               stroke="currentColor"
               strokeWidth="2"
               strokeLinecap="round"
               strokeLinejoin="round"
          >
               <polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" />
          </svg>
     );
};
export const GithubIcon = () => {
     return (
          <svg
               xmlns="http://www.w3.org/2000/svg"
               width="24"
               height="24"
               viewBox="0 0 24 24"
               fill="none"
               stroke="currentColor"
               strokeWidth="2"
               strokeLinecap="round"
               strokeLinejoin="round"
          >
               <path d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.403 5.403 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65-.17.6-.22 1.23-.15 1.85v4" />
               <path d="M9 18c-4.51 2-5-2-7-2" />
          </svg>
     );
};

export const InstagramIcon = () => {
     return (
          <svg
               xmlns="http://www.w3.org/2000/svg"
               width="24"
               height="24"
               viewBox="0 0 24 24"
               fill="none"
               stroke="currentColor"
               strokeWidth="2"
               strokeLinecap="round"
               strokeLinejoin="round"
          >
               <rect width="20" height="20" x="2" y="2" rx="5" ry="5" />
               <path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z" />
               <line x1="17.5" x2="17.51" y1="6.5" y2="6.5" />
          </svg>
     );
};

export const ChevronDownIcon = () => {
     return (
          <svg
               xmlns="http://www.w3.org/2000/svg"
               width="24"
               height="24"
               viewBox="0 0 24 24"
               fill="none"
               stroke="currentColor"
               strokeWidth="2"
               strokeLinecap="round"
               strokeLinejoin="round"
          >
               <path d="m6 9 6 6 6-6" />
          </svg>
     );
};
export const WebDevelopmentIcon = () => {
     return (
          <svg
               fill="#000000"
               version="1.1"
               id="Capa_1"
               xmlns="http://www.w3.org/2000/svg"
               viewBox="0 0 419.931 419.931"
          >
               <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
               <g
                    id="SVGRepo_tracerCarrier"
                    strokeLinecap="round"
                    strokeLinejoin="round"
               ></g>
               <g id="SVGRepo_iconCarrier">
                    {" "}
                    <g>
                         {" "}
                         <g>
                              {" "}
                              <g>
                                   {" "}
                                   <path d="M282.895,352.367c-2.176-1.324-4.072-3.099-5.579-5.25c-0.696-0.992-1.284-2.041-1.771-3.125H28.282V100.276h335.624 v159.138c7.165,0.647,13.177,5.353,15.701,11.797c2.235-1.225,4.726-1.982,7.344-2.213c1.771-0.154,3.53-0.044,5.236,0.293 V39.561c0-12.996-10.571-23.569-23.566-23.569H23.568C10.573,15.992,0,26.565,0,39.561v309.146 c0,12.996,10.573,23.568,23.568,23.568h257.179c-2.007-4.064-2.483-8.652-1.302-13.066 C280.126,356.67,281.304,354.354,282.895,352.367z M338.025,55.569c0-4.806,3.896-8.703,8.702-8.703h8.702 c4.807,0,8.702,3.896,8.702,8.703v9.863c0,4.806-3.896,8.702-8.702,8.702h-8.702c-4.807,0-8.702-3.896-8.702-8.702V55.569z M297.56,55.569c0-4.806,3.896-8.703,8.702-8.703h8.703c4.807,0,8.702,3.896,8.702,8.703v9.863c0,4.806-3.896,8.702-8.702,8.702 h-8.703c-4.806,0-8.702-3.896-8.702-8.702V55.569z M257.094,55.569c0-4.806,3.897-8.703,8.702-8.703h8.702 c4.807,0,8.703,3.896,8.703,8.703v9.863c0,4.806-3.896,8.702-8.703,8.702h-8.702c-4.805,0-8.702-3.896-8.702-8.702V55.569z"></path>{" "}
                                   <path d="M419.875,335.77l-2.615-14.83c-0.353-1.997-2.256-3.331-4.255-2.979l-13.188,2.324c-1.583-3.715-3.605-7.195-6.005-10.38 l8.614-10.268c0.626-0.744,0.931-1.709,0.847-2.68c-0.086-0.971-0.554-1.867-1.3-2.494l-11.534-9.68 c-0.746-0.626-1.713-0.93-2.683-0.845c-0.971,0.085-1.867,0.552-2.493,1.298l-8.606,10.26c-3.533-1.8-7.312-3.188-11.271-4.104 v-13.392c0-2.028-1.645-3.674-3.673-3.674h-15.06c-2.027,0-3.673,1.646-3.673,3.674v13.392 c-3.961,0.915-7.736,2.304-11.271,4.104l-8.608-10.259c-1.304-1.554-3.62-1.756-5.175-0.453l-11.535,9.679 c-0.746,0.627-1.213,1.523-1.299,2.494c-0.084,0.971,0.22,1.937,0.846,2.683l8.615,10.266c-2.396,3.184-4.422,6.666-6.005,10.38 l-13.188-2.325c-1.994-0.351-3.901,0.982-4.255,2.979l-2.614,14.83c-0.169,0.959,0.05,1.945,0.607,2.744 c0.561,0.799,1.41,1.342,2.37,1.511l13.198,2.326c0.215,4.089,0.927,8.045,2.073,11.812l-11.6,6.695 c-0.844,0.485-1.459,1.289-1.712,2.229c-0.252,0.941-0.119,1.943,0.367,2.787l7.529,13.041c0.485,0.844,1.289,1.459,2.229,1.711 c0.313,0.084,0.632,0.125,0.951,0.125c0.639,0,1.272-0.166,1.836-0.492l11.609-6.703c2.73,2.925,5.812,5.517,9.18,7.709 l-4.584,12.593c-0.332,0.916-0.289,1.926,0.123,2.809s1.157,1.566,2.072,1.898l14.148,5.149c0.406,0.148,0.832,0.224,1.257,0.224 c0.53,0,1.063-0.115,1.554-0.345c0.883-0.411,1.564-1.157,1.897-2.073l4.583-12.593c1.965,0.238,3.965,0.361,5.994,0.361 s4.029-0.125,5.994-0.361l4.584,12.593c0.332,0.916,1.016,1.662,1.897,2.073c0.49,0.229,1.021,0.345,1.554,0.345 c0.424,0,0.85-0.074,1.256-0.224l14.15-5.149c0.913-0.332,1.659-1.017,2.07-1.898c0.412-0.883,0.456-1.893,0.123-2.809 l-4.584-12.591c3.365-2.192,6.447-4.786,9.18-7.709l11.609,6.703c0.563,0.324,1.197,0.492,1.836,0.492 c0.318,0,0.64-0.043,0.951-0.125c0.941-0.252,1.743-0.869,2.229-1.711l7.529-13.043c0.486-0.842,0.619-1.846,0.367-2.787 c-0.253-0.938-0.868-1.742-1.712-2.229l-11.598-6.693c1.146-3.768,1.856-7.724,2.071-11.812l13.198-2.327 c0.96-0.169,1.812-0.712,2.37-1.511C419.825,337.715,420.044,336.729,419.875,335.77z M354.184,359.336 c-11.155,0-20.2-9.045-20.2-20.201s9.046-20.2,20.2-20.2c11.156,0,20.201,9.044,20.201,20.2S365.34,359.336,354.184,359.336z"></path>{" "}
                                   <g>
                                        {" "}
                                        <path d="M164.695,235.373c0-4.752-2.785-9.117-7.096-11.119l-39.455-18.332l39.456-18.334c4.31-2.004,7.095-6.368,7.095-11.118 v-0.319c0-4.21-2.119-8.075-5.665-10.334c-1.962-1.253-4.247-1.916-6.606-1.916c-1.778,0-3.563,0.391-5.16,1.133l-63.078,29.333 c-4.309,2.004-7.092,6.368-7.092,11.117v0.877c0,4.743,2.782,9.104,7.093,11.118l63.084,29.336 c1.631,0.755,3.368,1.138,5.162,1.138c2.338,0,4.616-0.664,6.597-1.924c3.548-2.268,5.666-6.13,5.666-10.335L164.695,235.373 L164.695,235.373z"></path>{" "}
                                        <path d="M226.932,134.012c-2.301-3.15-6.002-5.03-9.901-5.03h-0.314c-5.354,0-10.048,3.425-11.679,8.516L163.478,266.27 c-1.183,3.718-0.517,7.813,1.781,10.962c2.301,3.148,6.002,5.029,9.901,5.029h0.315c5.352,0,10.043-3.426,11.672-8.516 l41.555-128.762C229.896,141.268,229.234,137.167,226.932,134.012z"></path>{" "}
                                        <path d="M308.001,194.366l-63.079-29.333c-1.592-0.74-3.374-1.131-5.152-1.131c-2.358,0-4.644,0.661-6.605,1.912 c-3.552,2.263-5.671,6.127-5.671,10.337v0.319c0,4.746,2.783,9.111,7.097,11.123l39.454,18.33l-39.455,18.331 c-4.311,2.002-7.096,6.367-7.096,11.119v0.321c0,4.205,2.119,8.066,5.669,10.336c1.974,1.258,4.254,1.923,6.595,1.923 c1.792,0,3.527-0.383,5.169-1.141l63.082-29.336c4.307-2.009,7.088-6.371,7.088-11.114v-0.877 C315.094,200.735,312.311,196.371,308.001,194.366z"></path>{" "}
                                   </g>{" "}
                              </g>{" "}
                         </g>{" "}
                    </g>{" "}
               </g>
          </svg>
     );
};
export const SmartHomeAutomationIcon = () => {
     return (
          <svg
               fill="#000000"
               version="1.1"
               id="Layer_1"
               xmlns="http://www.w3.org/2000/svg"
               viewBox="0 0 512 512"
          >
               <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
               <g
                    id="SVGRepo_tracerCarrier"
                    strokeLinecap="round"
                    strokeLinejoin="round"
               ></g>
               <g id="SVGRepo_iconCarrier">
                    {" "}
                    <path d="M247.07,32.744H20.837C9.348,32.744,0,42.092,0,53.581v404.837c0,11.489,9.348,20.837,20.837,20.837H247.07 c11.489,0,20.837-9.348,20.837-20.837V53.581C267.907,42.092,258.559,32.744,247.07,32.744z M20.837,50.605H247.07 c1.613,0,2.977,1.363,2.977,2.977v50.605H17.86V53.581C17.86,51.968,19.224,50.605,20.837,50.605z M250.047,122.047V401.86H17.86 V122.047H250.047z M247.07,461.395H20.837c-1.613,0-2.977-1.363-2.977-2.977v-38.698h232.186v38.698 C250.047,460.032,248.683,461.395,247.07,461.395z"></path>{" "}
                    <rect
                         x="86.326"
                         y="68.465"
                         width="95.256"
                         height="17.86"
                    ></rect>{" "}
                    <rect
                         x="116.093"
                         y="431.628"
                         width="35.721"
                         height="17.86"
                    ></rect>{" "}
                    <rect
                         x="32.744"
                         y="139.907"
                         width="23.814"
                         height="17.86"
                    ></rect>{" "}
                    <rect
                         x="211.349"
                         y="139.907"
                         width="23.814"
                         height="17.86"
                    ></rect>{" "}
                    <rect
                         x="211.349"
                         y="366.14"
                         width="23.814"
                         height="17.86"
                    ></rect>{" "}
                    <rect
                         x="175.628"
                         y="366.14"
                         width="23.814"
                         height="17.86"
                    ></rect>{" "}
                    <rect
                         x="32.744"
                         y="366.14"
                         width="23.814"
                         height="17.86"
                    ></rect>{" "}
                    <rect
                         x="68.465"
                         y="366.14"
                         width="23.814"
                         height="17.86"
                    ></rect>{" "}
                    <path d="M407.814,218.744L303.628,335.953v143.303H512V335.953L407.814,218.744z M483.184,330.419h-150.74l75.37-84.791 L483.184,330.419z M428.651,461.395h-41.674v-53.581h41.674V461.395z M446.512,461.395v-53.581h14.884v-17.86H354.233v17.86h14.884 v53.581h-47.628V348.279H494.14v113.116H446.512z"></path>{" "}
                    <rect
                         x="395.907"
                         y="288.744"
                         width="23.814"
                         height="17.86"
                    ></rect>{" "}
                    <rect
                         x="389.953"
                         y="360.186"
                         width="35.721"
                         height="17.86"
                    ></rect>{" "}
                    <path d="M405.79,80.372c48.474,0,64.461,40.157,65.105,41.834l16.723-6.273c-0.819-2.181-20.737-53.422-81.828-53.422 c-31.907,0-51.93,14.523-63.108,26.707c-12.07,13.155-16.614,26.455-16.801,27.015l16.923,5.709 C343.389,120.245,357.68,80.372,405.79,80.372z"></path>{" "}
                    <path d="M434.221,146.02l16.723-6.273c-0.454-1.209-11.49-29.608-45.155-29.608c-33.781,0-43.78,28.686-44.188,29.907l16.935,5.675 c0.06-0.177,6.181-17.721,27.254-17.721C426.866,128,433.956,145.35,434.221,146.02z"></path>{" "}
                    <rect
                         x="395.907"
                         y="163.721"
                         width="23.814"
                         height="17.86"
                    ></rect>{" "}
                    <path d="M53.581,276.837v53.581h71.442v20.837h17.86v-20.837h71.442v-53.581h-23.814v-62.512c0-11.489-9.348-20.837-20.837-20.837 h-8.93v-26.791c0-9.667-7.668-14.884-14.884-14.884h-23.814c-9.667,0-14.884,7.668-14.884,14.884v26.791h-8.93 c-11.489,0-20.837,9.348-20.837,20.837v62.512H53.581z M196.465,312.558H71.442v-17.86h125.023V312.558z M125.023,169.674h17.86 v23.814h-17.86V169.674z M95.256,214.326c0-1.613,1.363-2.977,2.977-2.977h71.442c1.613,0,2.977,1.363,2.977,2.977v62.512H95.256 V214.326z"></path>{" "}
                    <rect
                         x="116.093"
                         y="235.163"
                         width="35.721"
                         height="17.86"
                    ></rect>{" "}
               </g>
          </svg>
     );
};
export const DigitalMarketingIcon = () => {
     return (
          <svg
               viewBox="0 0 64 64"
               fill="none"
               xmlns="http://www.w3.org/2000/svg"
               stroke="#000000"
          >
               <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
               <g
                    id="SVGRepo_tracerCarrier"
                    strokeLinecap="round"
                    strokeLinejoin="round"
               ></g>
               <g id="SVGRepo_iconCarrier">
                    {" "}
                    <path
                         fillRule="evenodd"
                         clipRule="evenodd"
                         d="M19.4687 37.7077C16.2323 35.9821 14.9779 31.7555 15.1033 27.804C22.2284 25.4031 28.5757 21.9517 33.4679 15.9495C38.5608 20.6513 40.6682 25.7032 40.8689 34.1314C32.063 32.9559 25.4146 35.0567 19.4687 37.7077Z"
                         fill="#00000000000"
                    ></path>{" "}
                    <path
                         fillRule="evenodd"
                         clipRule="evenodd"
                         d="M15.4546 31.2803C15.4546 31.2803 11.8921 32.6058 13.0211 35.2568C14.15 37.9078 17.562 36.0071 17.562 36.0071C16.5334 34.6065 15.8058 32.9809 15.4546 31.2803Z"
                         fill="#00000000000"
                    ></path>{" "}
                    <path
                         fillRule="evenodd"
                         clipRule="evenodd"
                         d="M22.8557 36.2322C24.6369 39.4834 25.791 43.0347 26.2676 46.6861L32.615 43.835C30.7584 40.9339 29.2782 37.8328 28.2245 34.5565C26.3931 34.9817 24.5867 35.5569 22.8557 36.2322Z"
                         fill="#00000000000"
                    ></path>{" "}
                    <path
                         fillRule="evenodd"
                         clipRule="evenodd"
                         d="M34.2457 17.7752C34.2457 17.7752 41.1198 15.0241 43.453 20.8514C45.6357 26.3284 39.0124 29.4046 39.0124 29.4046C38.4856 27.2038 36.5789 20.6513 34.2457 17.7752Z"
                         fill="#0000000000000000"
                    ></path>{" "}
                    <path
                         fillRule="evenodd"
                         clipRule="evenodd"
                         d="M33.9697 17.0999L33.0414 17.475L33.6686 18.2503C35.9517 21.0514 37.7831 27.4288 38.2849 29.5796L38.4856 30.455L39.3135 30.0798C40.5177 29.4796 41.6216 28.6793 42.575 27.7039C43.5283 26.8286 44.1806 25.6531 44.4566 24.4027C44.7325 23.1272 44.6071 21.8017 44.1304 20.6012C43.6788 19.3507 42.8509 18.2503 41.747 17.5C40.6432 16.7497 39.3386 16.3746 37.9838 16.3996C36.6291 16.3996 35.2743 16.6247 33.9697 17.0999ZM35.4499 18.1503C36.2778 17.9502 37.1308 17.8502 37.9587 17.8502C39.0124 17.8001 40.041 18.1003 40.894 18.7005C41.747 19.2757 42.4244 20.126 42.7757 21.1014C43.152 22.0518 43.2523 23.0771 43.0265 24.0775C42.8008 25.0779 42.2739 25.9783 41.5213 26.6535C40.9191 27.2787 40.2418 27.829 39.4891 28.2791C38.611 24.7528 37.2312 21.3265 35.4499 18.1503Z"
                         fill="#00000000000"
                    ></path>{" "}
                    <path
                         fillRule="evenodd"
                         clipRule="evenodd"
                         d="M20.7482 35.1318C17.5119 33.4062 16.2574 29.1795 16.3829 25.228C23.5079 22.8271 29.8552 19.3758 34.7474 13.3735C39.8404 18.0753 41.9478 23.1272 42.1485 31.5554C33.3425 30.405 26.6941 32.5058 20.7482 35.1318Z"
                         fill="white"
                    ></path>{" "}
                    <path
                         fillRule="evenodd"
                         clipRule="evenodd"
                         d="M20.397 35.782L20.7231 35.9571L21.0493 35.807C26.8948 33.206 33.4177 31.1303 42.0481 32.2807L42.9011 32.3807V31.5304C42.7004 22.8521 40.5177 17.6501 35.2743 12.8233L34.6972 12.2981L34.1955 12.8983C29.3785 18.8006 23.1567 22.1769 16.1571 24.5277L15.6804 24.7028V25.203C15.5298 29.4546 16.9599 33.9313 20.4471 35.782H20.397ZM20.7984 34.3065C18.139 32.6808 17.0853 29.1545 17.1104 25.7532C23.9093 23.4023 30.0057 20.051 34.7976 14.4489C39.1629 18.7005 41.0947 23.3523 41.3707 30.7301C32.9662 29.8048 26.5687 31.7805 20.7984 34.3065Z"
                         fill="#00000000000"
                    ></path>{" "}
                    <path
                         fillRule="evenodd"
                         clipRule="evenodd"
                         d="M16.6087 28.7544C16.6087 28.7544 13.0211 30.0799 14.1751 32.7309C15.3041 35.3819 18.7161 33.4812 18.7161 33.4812C17.6875 32.0807 16.9599 30.455 16.6087 28.7544Z"
                         fill="#000000"
                    ></path>{" "}
                    <path
                         fillRule="evenodd"
                         clipRule="evenodd"
                         d="M17.3363 28.6044L17.1857 27.754L16.3578 28.0541C15.6804 28.3292 15.0532 28.7044 14.5013 29.1796C13.9243 29.6047 13.5228 30.23 13.3472 30.9052C13.1716 31.6055 13.2218 32.3308 13.5228 32.981C13.7236 33.5312 14.0999 33.9814 14.6016 34.3065C15.0783 34.6316 15.6553 34.7817 16.2324 34.7817C17.2108 34.7567 18.1893 34.5316 19.0673 34.1064L19.8451 33.7313L19.3182 33.031C18.3398 31.7055 17.6624 30.205 17.3363 28.6044ZM16.1069 29.8048C16.4582 30.9552 16.9348 32.0807 17.5871 33.1061C17.3112 33.2061 17.0352 33.2561 16.7592 33.3061C16.3829 33.4062 15.9815 33.3812 15.6553 33.2061C15.3041 33.056 15.0281 32.7809 14.8525 32.4308C14.702 32.0557 14.6769 31.6555 14.8023 31.2804C14.9027 30.9052 15.1536 30.5801 15.4546 30.33C15.6553 30.1549 15.8811 29.9799 16.1069 29.8548V29.8048Z"
                         fill="#00000000000"
                    ></path>{" "}
                    <path
                         fillRule="evenodd"
                         clipRule="evenodd"
                         d="M24.2355 33.7562C26.0167 37.0074 27.1959 40.5588 27.6475 44.2102L33.9446 41.3591C32.0881 38.483 30.6079 35.3568 29.5542 32.0806C27.7227 32.5057 25.9415 33.0559 24.1853 33.7562H24.2355Z"
                         fill="white"
                    ></path>{" "}
                    <path
                         fillRule="evenodd"
                         clipRule="evenodd"
                         d="M23.9846 33.0811L23.2319 33.3812L23.5832 34.0814C25.3393 37.2326 26.4683 40.6839 26.9199 44.2353L27.0203 45.2857L35.0987 41.6593L34.6471 40.959C32.8156 38.133 31.3605 35.0568 30.3319 31.8556L30.1312 31.2303L29.4789 31.3804C27.5973 31.8056 25.7658 32.3808 23.9846 33.0811ZM25.2641 34.1315C26.5436 33.6563 27.8231 33.2561 29.1277 32.931C30.081 35.7571 31.3605 38.4831 32.916 41.0091L28.2496 43.1099C27.7478 39.9837 26.7192 36.9575 25.2641 34.1315Z"
                         fill="#00000000000"
                    ></path>{" "}
                    <path
                         fillRule="evenodd"
                         clipRule="evenodd"
                         d="M18.139 24.5027C18.2394 28.1791 19.6192 31.7055 22.0528 34.4815C22.2033 34.6316 22.3789 34.7066 22.5796 34.7066C22.7804 34.7066 22.956 34.6316 23.1065 34.4815C23.257 34.3314 23.3323 34.1564 23.3323 33.9563C23.3323 33.7562 23.257 33.5812 23.1065 33.4311C20.9238 30.9552 19.6945 27.779 19.6192 24.4777C19.6192 24.2776 19.544 24.1026 19.3934 23.9525C19.268 23.8275 19.0673 23.7274 18.8666 23.7274C18.6659 23.7274 18.4903 23.8024 18.3397 23.9525C18.2143 24.1026 18.139 24.2776 18.139 24.5027Z"
                         fill="#00000000000"
                    ></path>{" "}
                    <path
                         fillRule="evenodd"
                         clipRule="evenodd"
                         d="M39.9909 49.7874L36.9803 42.4596L38.7114 41.6343L38.0842 40.0587C41.5463 39.7586 48.27 39.2834 49.1481 41.2591C50.0261 43.2349 45.9869 48.887 43.3527 51.8382L41.6216 49.0121L39.9909 49.7874Z"
                         fill="#00000000000"
                    ></path>{" "}
                    <path
                         fillRule="evenodd"
                         clipRule="evenodd"
                         d="M33.9697 45.4107C35.4499 44.2102 37.1308 43.2348 38.9372 42.5846L39.6146 42.3345L39.0877 40.959L38.4103 41.2341C36.4534 41.9594 34.6471 42.9848 33.0414 44.3103L32.4895 44.7854L33.4178 45.9109L33.9697 45.4107Z"
                         fill="#00000000000"
                    ></path>{" "}
                    <path
                         fillRule="evenodd"
                         clipRule="evenodd"
                         d="M35.199 47.0863C36.5789 46.1109 38.1092 45.3607 39.6898 44.8355L40.3672 44.5854L39.8654 43.2098L39.188 43.4599C37.4569 44.0352 35.8011 44.8605 34.3209 45.9109L33.7439 46.336L34.5969 47.5115L35.199 47.0863Z"
                         fill="#00000000000"
                    ></path>{" "}
                    <path
                         fillRule="evenodd"
                         clipRule="evenodd"
                         d="M36.3029 48.862C37.6326 48.1117 39.0626 47.5115 40.5428 47.0863L41.2453 46.8612L40.7937 45.4607L40.0912 45.6858C38.4856 46.1359 36.9552 46.7862 35.5252 47.6115L34.8979 47.9867L35.6757 49.2371L36.3029 48.862Z"
                         fill="#00000000000"
                    ></path>{" "}
                    <path
                         fillRule="evenodd"
                         clipRule="evenodd"
                         d="M41.0446 47.6115L38.034 40.2837L39.7651 39.4334L38.7114 36.6323C42.1735 36.3322 49.2986 36.9074 50.2519 39.0582C51.2053 41.2091 46.9403 46.6861 44.281 49.6122L42.7255 46.8112L41.0697 47.5615L41.0446 47.6115Z"
                         fill="white"
                    ></path>{" "}
                    <path
                         fillRule="evenodd"
                         clipRule="evenodd"
                         d="M37.1057 39.9586L40.6682 48.6119L42.3993 47.8116L44.1304 50.9628L44.8078 50.1875C47.0657 47.7616 48.9975 45.0606 50.5279 42.1094C51.0798 41.109 51.2053 39.9586 50.9042 38.8582C50.6032 38.383 50.2268 37.9578 49.7753 37.6077C49.3237 37.2826 48.7968 37.0325 48.2449 36.9074C45.1339 36.0571 41.8976 35.732 38.6612 35.9821L37.6827 36.0821L38.8619 39.1583L37.1057 40.0086V39.9586ZM41.446 46.6362L38.9873 40.6839L40.7184 39.8586L39.7651 37.3576C42.4746 37.2326 45.159 37.5577 47.7682 38.283C48.2198 38.433 48.6463 38.6081 49.0477 38.8582C49.2735 38.9832 49.4491 39.1833 49.5745 39.4084C49.7 39.7835 49.7 40.2087 49.5745 40.5839C49.3738 41.2841 49.0728 41.9344 48.7215 42.5596C47.5173 44.6354 46.1124 46.5861 44.5318 48.3868L43.0767 45.9109L41.446 46.6362Z"
                         fill="#00000000000"
                    ></path>{" "}
                    <path
                         fillRule="evenodd"
                         clipRule="evenodd"
                         d="M43.1771 40.7839C43.6287 40.7839 44.0551 40.959 44.3562 41.2591C44.6573 41.5592 44.8329 41.9844 44.8329 42.4346C44.8329 42.8847 44.6573 43.3099 44.3562 43.61C44.0551 43.9101 43.6287 44.0852 43.1771 44.0852C42.7255 44.0852 42.299 43.9101 41.9979 43.61C41.6969 43.3099 41.5212 42.8847 41.5212 42.4346C41.5212 41.9844 41.6969 41.5592 41.9979 41.2591C42.3241 40.959 42.7506 40.7839 43.1771 40.7839Z"
                         fill="#0000000000000000"
                    ></path>{" "}
                    <path
                         fillRule="evenodd"
                         clipRule="evenodd"
                         d="M43.1771 40.0586C42.7004 40.0586 42.2237 40.1836 41.8474 40.4587C41.446 40.7088 41.1449 41.084 40.9442 41.5342C40.7686 41.9843 40.7184 42.4595 40.7937 42.9097C40.894 43.3849 41.1198 43.81 41.446 44.1351C41.7721 44.4603 42.2237 44.7104 42.6753 44.7854C43.152 44.8854 43.6287 44.8354 44.0802 44.6603C44.5318 44.4853 44.9082 44.1602 45.159 43.785C45.435 43.3849 45.5605 42.9347 45.5605 42.4595C45.5605 41.8343 45.3096 41.234 44.858 40.7839C44.4064 40.3087 43.8043 40.0586 43.1771 40.0586ZM43.1771 41.5091C43.3025 41.5091 43.428 41.5342 43.5283 41.5842C43.6287 41.6342 43.729 41.7092 43.8294 41.7842C43.9046 41.8593 43.9799 41.9593 44.0301 42.0844C44.0802 42.1844 44.1053 42.3094 44.0802 42.4345C44.0802 42.6096 44.0301 42.7846 43.9297 42.9347C43.8294 43.0847 43.6788 43.1848 43.5283 43.2598C43.3527 43.3348 43.1771 43.3348 43.0015 43.3098C42.8258 43.2848 42.6753 43.1848 42.5499 43.0597C42.4244 42.9347 42.3492 42.7846 42.299 42.6096C42.2739 42.4345 42.2739 42.2594 42.3492 42.0844C42.4244 41.9093 42.5248 41.7842 42.6753 41.6842C42.8258 41.5842 43.0015 41.5091 43.1771 41.5091Z"
                         fill="#00000000000"
                    ></path>{" "}
                    <path
                         fillRule="evenodd"
                         clipRule="evenodd"
                         d="M44.858 37.6078C46.6142 38.9833 47.718 42.1595 47.7682 44.2353C47.7682 44.4354 47.8435 44.6104 47.994 44.7605C48.1194 44.8855 48.3202 44.9856 48.5209 44.9856C48.7216 44.9856 48.8972 44.9105 49.0477 44.7605C49.1731 44.6354 49.2735 44.4354 49.2735 44.2353C49.2735 41.7843 47.8686 38.0829 45.8113 36.4823C45.7361 36.4323 45.6357 36.3823 45.5604 36.3573C45.4601 36.3323 45.3597 36.3323 45.2594 36.3323C45.159 36.3323 45.0587 36.3823 44.9834 36.4323C44.9082 36.4823 44.8329 36.5574 44.7576 36.6324C44.6573 36.7824 44.6071 36.9825 44.6322 37.1576C44.6071 37.3327 44.7074 37.5077 44.858 37.6078Z"
                         fill="#00000000000"
                    ></path>{" "}
               </g>
          </svg>
     );
};
export const UIDesignIcon = () => {
     return (
          <svg
               viewBox="0 0 24 24"
               fill="none"
               xmlns="http://www.w3.org/2000/svg"
          >
               <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
               <g
                    id="SVGRepo_tracerCarrier"
                    strokeLinecap="round"
                    strokeLinejoin="round"
               ></g>
               <g id="SVGRepo_iconCarrier">
                    {" "}
                    <path
                         fillRule="evenodd"
                         clipRule="evenodd"
                         d="M8.66683 1.25C6.41167 1.25 4.5835 3.07817 4.5835 5.33333C4.5835 6.70965 5.26442 7.92693 6.30776 8.66667C5.26442 9.4064 4.5835 10.6237 4.5835 12C4.5835 13.3763 5.26442 14.5936 6.30777 15.3333C5.26442 16.0731 4.5835 17.2903 4.5835 18.6667C4.5835 20.9218 6.41167 22.75 8.66683 22.75C10.922 22.75 12.7502 20.9218 12.7502 18.6667V15.1624C13.4539 15.738 14.3534 16.0833 15.3335 16.0833C17.5887 16.0833 19.4168 14.2552 19.4168 12C19.4168 10.6237 18.7359 9.4064 17.6926 8.66666C18.7359 7.92693 19.4168 6.70965 19.4168 5.33333C19.4168 3.07817 17.5887 1.25 15.3335 1.25H8.66683ZM11.2502 14.5833H8.66683C7.24009 14.5833 6.0835 13.4267 6.0835 12C6.0835 10.5733 7.24009 9.41666 8.66683 9.41667L11.2502 9.41666L11.2502 12L11.2502 14.5833ZM8.66683 7.91667H11.2502V2.75H8.66683C7.24009 2.75 6.0835 3.9066 6.0835 5.33333C6.0835 6.76007 7.24009 7.91667 8.66683 7.91667ZM17.9168 5.33333C17.9168 6.75914 16.7617 7.91517 15.3363 7.91667L14.0432 7.91666L12.7502 7.91666L12.7502 2.75H15.3335C16.7602 2.75 17.9168 3.9066 17.9168 5.33333ZM12.7502 11.9968C12.7519 10.5726 13.9061 9.41838 15.3303 9.41667L15.336 9.41667C16.7616 9.41801 17.9168 10.5741 17.9168 12C17.9168 13.4267 16.7602 14.5833 15.3335 14.5833C13.9078 14.5833 12.7519 13.4284 12.7502 12.0032V11.9968ZM8.66683 16.0833C7.24009 16.0833 6.0835 17.2399 6.0835 18.6667C6.0835 20.0934 7.24009 21.25 8.66683 21.25C10.0936 21.25 11.2502 20.0934 11.2502 18.6667V16.0833H8.66683Z"
                         fill="#000000"
                    ></path>{" "}
               </g>
          </svg>
     );
};
export const CyberSecurityIcon = () => {
     return (
          <svg
               viewBox="0 0 24 24"
               xmlns="http://www.w3.org/2000/svg"
               fill="#000000"
          >
               <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
               <g
                    id="SVGRepo_tracerCarrier"
                    strokeLinecap="round"
                    strokeLinejoin="round"
               ></g>
               <g id="SVGRepo_iconCarrier">
                    <path d="M21.15,5.89A11,11,0,1,0,23,12,11,11,0,0,0,21.15,5.89ZM19.48,17A9,9,0,1,1,21,12,9,9,0,0,1,19.48,17ZM15,11V9A3,3,0,0,0,9,9v2a1,1,0,0,0-1,1v4a1,1,0,0,0,1,1h6a1,1,0,0,0,1-1V12a1,1,0,0,0-1-1Zm-1.5,0h-3V9a1.5,1.5,0,0,1,3,0Z"></path>
                    <path
                         d="M19.29,15.06a.93.93,0,0,0,.18-.26.78.78,0,0,0,.07-.31.78.78,0,0,0-.07-.3.93.93,0,0,0-.18-.26.76.76,0,0,0-.28-.15.8.8,0,0,0-.31,0,.7.7,0,0,0-.37.11.77.77,0,0,0-.26.28H16v.7h2.05a.69.69,0,0,0,.27.29.73.73,0,0,0,.38.11.8.8,0,0,0,.31,0A.76.76,0,0,0,19.29,15.06ZM19,14.49a.27.27,0,0,1-.28.28.22.22,0,0,1-.11,0,.28.28,0,0,1-.15-.15.22.22,0,0,1,0-.11.27.27,0,0,1,.28-.28.2.2,0,0,1,.11,0,.21.21,0,0,1,.1.06.18.18,0,0,1,.06.09.2.2,0,0,1,0,.11Zm.26-3.94,1.32-1.32q.12.39.21.78l-1.25,1.24H17.79V12h-.7v-1.4ZM9.8,19h.69v1.14l-.55.56c-.26-.06-.52-.13-.77-.21l.63-.63Zm2.56-.61V17h-.7v1.43a.8.8,0,0,0-.29.26.7.7,0,0,0-.11.37.75.75,0,0,0,.75.75.87.87,0,0,0,.28,0,.82.82,0,0,0,.25-.16.78.78,0,0,0,.16-.24.67.67,0,0,0,.06-.28.74.74,0,0,0-.4-.65Zm-.15.83a.28.28,0,0,1-.48-.2.27.27,0,0,1,.28-.28.28.28,0,0,1,.2.48ZM21,12a1.49,1.49,0,0,1,0,.21l-1,1H16v-.7h3.7L21,11.25C21,11.5,21,11.75,21,12ZM14.84,8.09l3-3a5.8,5.8,0,0,1,.52.47L15,8.93A2.65,2.65,0,0,0,14.84,8.09ZM5.3,13.75a.82.82,0,0,0-.31,0,.76.76,0,0,0-.28.15.93.93,0,0,0-.18.26.78.78,0,0,0-.07.3.78.78,0,0,0,.07.31.93.93,0,0,0,.18.26.76.76,0,0,0,.28.15.82.82,0,0,0,.31,0,.74.74,0,0,0,.65-.4H8v-.7H6a.76.76,0,0,0-.3-.29A.79.79,0,0,0,5.3,13.75Zm.18.92a.27.27,0,0,1-.19.08l-.1,0a.21.21,0,0,1-.1-.06A.18.18,0,0,1,5,14.58a.2.2,0,0,1,0-.11.43.43,0,0,1,0-.11l.06-.09.09-.06.11,0a.28.28,0,0,1,.2.48Zm11.61,1.76L18.69,18c-.16.17-.33.32-.5.48l-1.38-1.37H15.65v1.54l1,1c-.21.12-.43.22-.65.33L15,19V17.13H14v1.44h-.7V17H15a.92.92,0,0,0,.89-.57ZM12,4.1a.75.75,0,0,0-.53.22.71.71,0,0,0-.22.52.81.81,0,0,0,.12.38.78.78,0,0,0,.29.28V6L12,6a2.2,2.2,0,0,1,.36,0V5.5a.78.78,0,0,0,.29-.28.7.7,0,0,0,.11-.38.79.79,0,0,0-.24-.52A.82.82,0,0,0,12,4.1Zm.23.9a.29.29,0,0,1-.12.1.22.22,0,0,1-.16,0,.24.24,0,0,1-.22-.22.22.22,0,0,1,0-.16.29.29,0,0,1,.1-.12A.3.3,0,0,1,12,4.56l.11,0,.09.06.06.09a.43.43,0,0,1,0,.11A.3.3,0,0,1,12.23,5ZM9,17h.05v1.94L8,20c-.22-.11-.42-.23-.63-.35l1-1V17.52l-.81.82H5.93v.28a3.58,3.58,0,0,1-.29-.26c-.15-.14-.27-.31-.41-.47v-.25h2l1-1a.56.56,0,0,0,0,.08A1,1,0,0,0,9,17ZM4.18,13.21,3,12.05c0-.33,0-.65,0-1l1.44,1.43H8v.7Zm1-4.8a.73.73,0,0,0,.2.35.72.72,0,0,0,1,0,.75.75,0,0,0,0-1,.75.75,0,0,0-.72-.19l-.9-.93c-.14.18-.27.36-.39.55L5.23,8A.71.71,0,0,0,5.22,8.41ZM5.74,8a.29.29,0,0,1,.38,0,.28.28,0,0,1,.08.2.27.27,0,0,1-.08.19.29.29,0,0,1-.38,0,.35.35,0,0,1-.06-.19A.36.36,0,0,1,5.74,8Zm2.35,8.41H4.43l-.16.16c-.12-.21-.24-.41-.34-.63l.23-.23H8V16A1.16,1.16,0,0,0,8.09,16.41ZM14,19.25,15,20.3v.15c-.23.08-.46.17-.7.23V20.6l-.86-.86ZM17.67,8a.68.68,0,0,0-.35.17.75.75,0,0,0,0,1,.75.75,0,0,0,1,0,.79.79,0,0,0,.18-.72l1.15-1.15c-.12-.19-.24-.38-.37-.56L18.05,8A.7.7,0,0,0,17.67,8Zm.46.68a.22.22,0,0,1,0,.11.41.41,0,0,1-.05.09.24.24,0,0,1-.09.06.31.31,0,0,1-.22,0,.24.24,0,0,1-.09-.06.29.29,0,0,1,0-.38.32.32,0,0,1,.19-.07.31.31,0,0,1,.18.07.27.27,0,0,1,.07.08A.38.38,0,0,1,18.13,8.66Zm1.36,7.17.42.42c-.12.21-.23.43-.36.63L19,16.32ZM4.72,11,3.33,9.63c.07-.24.15-.49.24-.73L5,10.32H6.44V9.67h.7V11Zm9.62-6.71h-.7V3.16c.24,0,.47.1.7.16Zm.56,1.51a.53.53,0,0,0,.28.07.72.72,0,0,0,.32-.08.67.67,0,0,0,.26-.21.73.73,0,0,0,.14-.31.74.74,0,0,0,0-.33l.7-.7-.65-.35-.54.54a.74.74,0,0,0-.39,0,.63.63,0,0,0-.33.2.75.75,0,0,0,0,1A.58.58,0,0,0,14.9,5.82ZM15,5a.29.29,0,0,1,.38,0,.24.24,0,0,1,.06.09.31.31,0,0,1,0,.22.24.24,0,0,1-.06.09.29.29,0,0,1-.38,0A.29.29,0,0,1,15,5ZM9.36,3.4l1.46,1.47V6.25a3,3,0,0,0-.7.43V5.15L8.65,3.67A7.46,7.46,0,0,1,9.36,3.4ZM8.12,5.51l-1-1c.2-.13.41-.24.63-.36l.9.9ZM6.19,5.15l3,3A3.36,3.36,0,0,0,9,9L5.68,5.62C5.84,5.46,6,5.3,6.19,5.15Z"
                         opacity="0.5"
                    ></path>
                    <rect width="24" height="24" fill="none"></rect>
               </g>
          </svg>
     );
};
export const GraphicDesignIcon = () => {
     return (
          <svg
               viewBox="0 0 32 32"
               fill="none"
               xmlns="http://www.w3.org/2000/svg"
          >
               <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
               <g
                    id="SVGRepo_tracerCarrier"
                    strokeLinecap="round"
                    strokeLinejoin="round"
               ></g>
               <g id="SVGRepo_iconCarrier">
                    {" "}
                    <path
                         d="M2 12.1333C2 8.58633 2 6.81283 2.69029 5.45806C3.29749 4.26637 4.26637 3.29749 5.45806 2.69029C6.81283 2 8.58633 2 12.1333 2H19.8667C23.4137 2 25.1872 2 26.5419 2.69029C27.7336 3.29749 28.7025 4.26637 29.3097 5.45806C30 6.81283 30 8.58633 30 12.1333V19.8667C30 23.4137 30 25.1872 29.3097 26.5419C28.7025 27.7336 27.7336 28.7025 26.5419 29.3097C25.1872 30 23.4137 30 19.8667 30H12.1333C8.58633 30 6.81283 30 5.45806 29.3097C4.26637 28.7025 3.29749 27.7336 2.69029 26.5419C2 25.1872 2 23.4137 2 19.8667V12.1333Z"
                         fill="#000000"
                    ></path>{" "}
                    <path
                         d="M16.6465 10.2338L13.1528 16.1482L16.8906 22.4285C16.9683 22.5709 16.8906 22.6024 16.778 22.6024H14.1108C13.9229 22.6024 13.7914 22.5956 13.7163 22.4671C13.4656 21.9662 13.2152 21.4685 12.9649 20.974C12.7143 20.4797 12.4482 19.9756 12.1667 19.4617C11.8849 18.9482 11.6063 18.4281 11.3308 17.9013C11.0802 18.4151 10.8015 18.9288 10.5325 19.4425C10.2631 19.9563 9.997 20.4668 9.73421 20.974C9.47126 21.4815 9.20203 21.9856 8.92655 22.4863C8.87636 22.6019 8.78858 22.6138 8.66353 22.6138H6.09026C5.98915 22.6138 5.97695 22.5378 6.03392 22.4478L9.65907 16.3408L6.1278 10.2145C6.06366 10.1271 6.11886 10.034 6.22174 10.0411H8.87015C8.98483 10.0347 9.08618 10.0779 9.15191 10.176C9.37732 10.6898 9.62776 11.2036 9.90323 11.7172C10.1785 12.231 10.4572 12.7384 10.7391 13.2391C11.0209 13.74 11.2995 14.2473 11.5374 14.7611C11.7877 14.2346 12.0413 13.7209 12.2981 13.2199C12.5547 12.719 12.8176 12.2149 13.087 11.7076C13.3562 11.2004 13.616 10.6963 13.8665 10.1953C13.8998 10.0854 13.9794 10.0297 14.0919 10.0411H16.5525C16.6465 10.0411 16.6853 10.1694 16.6465 10.2338Z"
                         fill="#ffffff"
                    ></path>{" "}
                    <path
                         d="M22.0371 22.8525C20.3806 22.8784 18.6455 22.1963 17.7733 20.6852C17.3475 19.9597 17.1346 19.0511 17.1347 17.9592C17.1276 17.075 17.3479 16.2045 17.7733 15.4355C18.871 13.4733 21.0824 12.7381 23.1829 12.9311V9.13586C23.1829 9.04615 23.2205 9.00101 23.2956 9.00101H25.6623C25.727 8.99153 25.7842 9.05023 25.775 9.11658V20.5022C25.775 20.9719 25.8216 21.44 25.8502 21.9085C25.8548 21.9885 25.8085 22.0677 25.7374 22.1012C24.5681 22.6014 23.3028 22.8467 22.0371 22.8525ZM23.1829 20.4636V15.2043C21.4673 14.7277 19.8017 16.0344 19.8019 17.8436C19.7612 19.7141 21.3736 21.0064 23.1829 20.4636Z"
                         fill="#ffffff"
                    ></path>{" "}
               </g>
          </svg>
     );
};
export const AppDevelopmentIcon = () => {
     return (
          <svg
               fill="#000000"
               viewBox="0 0 512 512"
               version="1.1"
               xmlns="http://www.w3.org/2000/svg"
          >
               <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
               <g
                    id="SVGRepo_tracerCarrier"
                    strokeLinecap="round"
                    strokeLinejoin="round"
               ></g>
               <g id="SVGRepo_iconCarrier">
                    {" "}
                    <g id="_x31_87_x2C__certificate_x2C__certification_x2C__App_x2C__application_x2C__aproval">
                         {" "}
                         <g>
                              {" "}
                              <path d="M363.393,372.136H112.79v50.975c0,4.159,1.729,7.932,4.443,10.681 c2.75,2.75,6.521,4.441,10.681,4.441H348.27c4.161,0,7.932-1.691,10.682-4.441c2.75-2.749,4.441-6.521,4.441-10.681V372.136z M245.159,397.975c-1.797-1.797-4.301-2.89-7.051-2.89c-2.785,0-5.287,1.093-7.085,2.927c-1.797,1.762-2.925,4.265-2.925,7.049 c0,2.75,1.128,5.253,2.925,7.051c1.798,1.798,4.301,2.926,7.085,2.926c2.645,0,5.077-1.021,6.875-2.714l0.175-0.212 c1.799-1.798,2.927-4.301,2.927-7.051C248.086,402.276,246.958,399.773,245.159,397.975z M238.108,386.413 c5.112,0,9.765,2.079,13.15,5.464c3.383,3.385,5.499,8.037,5.499,13.184s-2.116,9.801-5.464,13.185l-0.318,0.281 c-3.35,3.209-7.896,5.183-12.868,5.183c-5.146,0-9.835-2.08-13.218-5.464c-3.35-3.384-5.429-8.038-5.429-13.185 s2.079-9.799,5.429-13.184C228.273,388.492,232.962,386.413,238.108,386.413z M111.521,221.225h0.035h0.105l0.072-0.036h0.035 h0.105h0.141h0.247l0.283-0.035h0.07h0.21l1.128-0.105l0.212-0.036h0.246l0,0l0.282-0.035l0.141-0.035 c8.566-0.881,16.568-3.561,23.689-7.649l0.072-0.035l0.211-0.14l0.07-0.037l0.177-0.07l0.211-0.141l0,0l0.247-0.141l0.105-0.07 l0.105-0.07l0.247-0.142h0.035l0.177-0.142l0.176-0.105l0.035-0.035l0,0h0.035l0.035-0.035l0.318-0.212l0.07-0.035l0.07-0.034 l0.107-0.07l0.599-0.389l0.07-0.035l0.388-0.282l0.142-0.07l0.246-0.177l0.353-0.246l0.035-0.035l0.388-0.282l0.177-0.105 l0.211-0.142l0.388-0.282l0,0l0.388-0.281l0.177-0.142l0.21-0.141l0.353-0.282l0.035-0.035l0.565-0.423l0.14-0.141l0.388-0.282 l0.035-0.036l0.318-0.246l0.247-0.212l0.141-0.104l0.353-0.283l0.07-0.07l0.283-0.246l0.246-0.248l0.107-0.07l0.704-0.634 l0.283-0.247l0.07-0.07l0.353-0.316l0.105-0.107l0.248-0.211l0.283-0.281l0.035-0.036l0.353-0.317l0.14-0.141l0.528-0.493v-0.036 l0.318-0.316l0.177-0.176l0.176-0.178l0.317-0.352l0,0l0.318-0.317l0.175-0.212l0.142-0.141l0.316-0.353l0.035-0.035l0.283-0.317 l0.212-0.212l0.105-0.142l1.233-1.409l0.07-0.105l0.212-0.282l0.246-0.282l0.072-0.069l0.281-0.389l0.107-0.105l0.175-0.247 l0.248-0.317l0.035-0.07l0.317-0.353l0.105-0.177l0.175-0.211l0.248-0.354l0.035-0.035l0.247-0.387l0.141-0.177l0.14-0.212 l0.283-0.387l0,0l0.248-0.389l0.281-0.388l0.247-0.388l0.035-0.035l0.248-0.353l0.141-0.247l0.105-0.141l0.248-0.424l0.035-0.069 l0.212-0.317l0.175-0.282l0.07-0.142l0.247-0.387l0.07-0.105l0.353-0.636l0.07-0.069l0.635-1.199l0.035-0.035l0.21-0.423 l0.107-0.177l0.105-0.246l0.21-0.388l0.035-0.036l0.177-0.387l0.141-0.248l0.07-0.176l0.212-0.423l0.035-0.07l0.176-0.317 l0.142-0.354l0.035-0.07l0.21-0.422l0.072-0.177l0.105-0.247l0.388-0.881l0.07-0.212l0.105-0.212l0.177-0.422v-0.035l0.176-0.389 l0.105-0.316l0.072-0.105l0.175-0.459l0.035-0.105l0.105-0.318l0.142-0.387l0.035-0.07l0.14-0.424l0.07-0.176l0.107-0.281 l0.14-0.424l0,0l0.141-0.459l0.107-0.246l0.035-0.211l0.141-0.424l0.035-0.07l0.107-0.388l0.105-0.317l0.035-0.141l0.141-0.458 l0.035-0.142l0.107-0.317l0.105-0.388v-0.07l0.105-0.458l0.07-0.212l0.07-0.247l0.107-0.458l0,0l0.105-0.458l0.07-0.282 l0.035-0.177l0.107-0.458l0.035-0.105l0.07-0.352l0.07-0.389l0.035-0.105l0.07-0.459l0.035-0.141l0.072-0.316l0.07-0.459v-0.035 l0.105-0.459l0.035-0.246l0.035-0.211l0.07-0.494v-0.035l0.072-0.423l0.035-0.317l0.035-0.177l0.07-0.458v-0.141l0.035-0.354 l0.07-0.388v-0.104l0.07-0.459v-0.211l0.035-0.283l0.035-0.457v-0.035l0.037-0.494l0.035-0.246v-0.212l0.035-0.494v-0.07 l0.035-0.423l0.035-0.353v-0.142v-0.492l0.035-0.142v-0.353v-0.424l0.035-0.07v-0.493v-0.212v-0.281v-0.494 c0-16.674-6.768-31.761-17.662-42.689c-10.927-10.928-26.049-17.695-42.725-17.695c-16.568,0-31.585,6.697-42.477,17.484 l-0.212,0.211c-10.927,10.929-17.695,26.016-17.695,42.689c0,16.675,6.768,31.762,17.695,42.689 c3.137,3.138,6.627,5.959,10.365,8.319l0.105,0.036l0.07,0.07l0.07,0.035l0.07,0.035l0.072,0.07 c8.037,5.006,17.307,8.179,27.285,8.954h0.035l0.281,0.035h0.248h0.212l0.07,0.035h0.281h0.248l0.175,0.036h0.107h0.281 l0.248,0.035h0.141h0.424h0.246h0.105h0.177l0.283,0.035h0.281h0.035h0.247h0.248h0.281h0.072h0.035h0.035h0.07h0.035h0.035l0,0 h0.07h0.035h0.07h0.037h0.035h0.035h0.035h0.035h0.035h0.07h0.035h0.07l0,0h0.037h0.07h0.035h0.035h0.07l0,0h0.035h0.035h0.07 h0.037h0.07h0.035v-0.035h0.035h0.07h0.035h0.035h0.07h0.037l0,0h0.07h0.035h0.105h0.035h0.142h0.035h0.07h0.035l0,0h0.07h0.035 h0.035h0.07h0.035h0.037l0,0h0.07l0.423-0.035h0.035l0,0H111.521z M147.195,296.627c-0.316,2.045-2.114,3.666-4.264,3.666 c-1.235,0-2.362-0.529-3.173-1.375l-0.037-0.035l-0.035-0.035l-26.897-26.191v90.807h250.603V41.124 c0-4.125-1.691-7.932-4.441-10.646c-2.75-2.749-6.521-4.476-10.682-4.476H127.914c-4.16,0-7.931,1.727-10.681,4.476 c-2.714,2.715-4.443,6.521-4.443,10.646v25.839h217.644c2.361,0,4.301,1.939,4.301,4.336c0,2.397-1.939,4.336-4.301,4.336H112.79 v16.357l0.107,0.035l0.704,0.035h0.177l0.634,0.07h0.247l0.563,0.07l0.283,0.035l0.53,0.035l0.351,0.035l0.423,0.07l0.459,0.036 l0.353,0.07l0.493,0.07l0.318,0.036l0.528,0.07l0.74,0.105l0.141,0.035l0.177,0.035l0.668,0.105l0.107,0.036l0.739,0.14h0.037 l0.774,0.142h0.037l0.739,0.177h0.072l0.704,0.176l0.142,0.035l0.634,0.142l0.212,0.035l0.563,0.142l0.248,0.07l0.493,0.141 l0.353,0.105l0.423,0.105l0.388,0.105l0.353,0.106l0.458,0.141l0.317,0.071l0.493,0.176l0.248,0.07l0.563,0.176l0.177,0.035 l0.633,0.212l0.107,0.035l0.704,0.247l0.035,0.035l0.74,0.247l0,0l0.741,0.281l0.07,0.036l0.635,0.246l0.141,0.035l0.6,0.247 l0.175,0.071l0.53,0.211l0.246,0.141l0.775,0.317l0.388,0.177l0.388,0.176l0.353,0.142l0.388,0.212l0.283,0.105l0.493,0.246 l0.212,0.105l0.528,0.247l0.176,0.071l0.565,0.316l0.105,0.035l0.67,0.353h0.035l0.67,0.388l0,0l1.339,0.74l0.14,0.07l0.53,0.318 l0.175,0.105l0.493,0.282l0.212,0.14l0.458,0.283l0.247,0.141l0.388,0.247l0.315,0.212l0.318,0.211l0.388,0.246l0.283,0.177 l0.423,0.282l0.212,0.141l0.458,0.318l0.175,0.104l0.528,0.388l0.072,0.071l0.598,0.423l0.035,0.035l0.6,0.458h0.035l0.634,0.494 l0.565,0.423l0.105,0.07l0.493,0.388l0.142,0.142l0.458,0.352l0.211,0.177l0.388,0.317l0.247,0.211l0.318,0.283l0.283,0.246 l0.315,0.246l0.318,0.318l0.248,0.211l0.388,0.317l0.175,0.177l0.423,0.387l0.142,0.141l0.458,0.424l0.105,0.105l0.493,0.494 l0.035,0.035l0.565,0.528c12.515,12.515,20.234,29.788,20.234,48.823v0.564v0.317l-0.035,0.247v0.563v0.105v0.459l-0.035,0.423 v0.141l-0.035,0.564v0.176v0.388l-0.035,0.494v0.07l-0.035,0.528l-0.035,0.282l-0.037,0.282l-0.035,0.564v0.035l-0.035,0.493 l-0.035,0.353l-0.035,0.211l-0.07,0.564v0.105l-0.035,0.424l-0.07,0.458v0.104l-0.072,0.529l-0.035,0.212l-0.07,0.353l-0.07,0.493 v0.036l-0.07,0.564l-0.07,0.281l-0.035,0.246l-0.107,0.529v0.07l-0.07,0.494l-0.07,0.353l-0.035,0.175l-0.105,0.529l-0.037,0.142 l-0.07,0.388l-0.105,0.424l-0.035,0.104l-0.105,0.529l-0.035,0.212l-0.107,0.316l-0.105,0.529l0,0l-0.14,0.528l-0.07,0.282 l-0.072,0.247l-0.141,0.528v0.07l-0.14,0.459l-0.107,0.353l-0.035,0.142l-0.141,0.527l-0.035,0.142l-0.142,0.388l-0.105,0.423 l-0.035,0.071l-0.176,0.527l-0.072,0.212l-0.07,0.282l-0.175,0.529l0,0l-0.177,0.493l-0.105,0.317l-0.07,0.176l-0.175,0.529 l-0.037,0.07l-0.176,0.424l-0.105,0.387l-0.07,0.105l-0.177,0.494l-0.07,0.176l-0.14,0.318l-0.177,0.457l-0.035,0.036 l-0.176,0.493l-0.107,0.246l-0.105,0.248l-0.211,0.493l-0.037,0.034l-0.176,0.459l-0.14,0.317l-0.107,0.177l-0.211,0.494 l-0.035,0.069l-0.177,0.388l-0.176,0.389l-0.07,0.104l-0.212,0.459l-0.105,0.177l-0.14,0.316l-0.495,0.952l-0.105,0.212 l-0.14,0.246l-0.248,0.459l-0.035,0.035l-0.212,0.422l-0.176,0.318l-0.317,0.6l-0.105,0.141l-0.177,0.281l-0.246,0.424l0,0 l-0.248,0.457l-0.175,0.247l-0.107,0.177l-0.246,0.459l-0.07,0.07l-0.212,0.352l-0.248,0.354l-0.035,0.07l-0.246,0.422 l-0.177,0.212l-0.105,0.212l-0.283,0.423l-0.07,0.071l-0.246,0.352l-0.212,0.317l-0.07,0.106l-0.283,0.423l-0.14,0.177 l-0.143,0.211l-0.315,0.423v0.035l-0.283,0.388l-0.212,0.281l-0.105,0.106l-0.318,0.388l-0.105,0.142l-0.175,0.281l-0.317,0.388 l0,0l-0.318,0.424l-0.212,0.211l-0.105,0.176l-0.318,0.389l-0.07,0.105l-0.246,0.281l-0.282,0.354l-0.07,0.035l-0.317,0.388 l-0.177,0.177l-0.176,0.211l-0.317,0.388l-0.035,0.035l-0.318,0.317l-0.246,0.316l-0.072,0.072l-0.704,0.739l-0.353,0.388l0,0 l-0.353,0.353l-0.811,0.846l-0.565,0.564l-0.035,0.035l-0.388,0.353l-0.176,0.176l-0.177,0.142l-0.351,0.353l-0.107,0.105 l-0.281,0.246l-0.317,0.283l-0.035,0.07c-0.635,0.564-1.271,1.128-1.939,1.657l-0.388,0.317l-0.142,0.141l-0.246,0.211 c-1.058,0.846-2.116,1.656-3.208,2.468l-0.423,0.282l-0.072,0.07l-1.162,0.811v77.941v0.035v0.106v0.07l0,0l-0.037,0.035v0.317 l0,0L147.195,296.627L147.195,296.627z M105.316,261.657l0.035-0.07l0.037-0.035l0.035-0.036l1.304-1.162l0.07-0.071l0.07-0.069 h0.035l0.072-0.07l0,0l0.07-0.037v-0.035l0.07-0.034l0.035-0.035l0.07-0.035h0.035l0.035-0.071h0.035l0.072-0.035l0.035-0.035 l0.141-0.07h0.035l0.035-0.035l0.142-0.07l0.07-0.035h0.035l0.035-0.035l0.248-0.106h0.035l0.21-0.07h0.07h0.037l0.351-0.106 h0.035h0.072l0,0c0.423-0.07,0.811-0.07,1.199-0.035h0.035l0.175,0.035l0,0l0.107,0.035l0,0h0.105l0.07,0.036h0.035l0.07,0.035 h0.035l0.07,0.035h0.037h0.07v0.035h0.07l0.035,0.035h0.07l0.318,0.142l0.07,0.035h0.035l0.035,0.035l0.177,0.105l0.07,0.035 l0.035,0.035l0.141,0.071l0.037,0.035l0.246,0.177l0.035,0.035l0.035,0.035l0.177,0.141l0.035,0.035l0.035,0.035l0.07,0.035 l0.07,0.105l25.805,25.1v-62.643l-0.177,0.07l-0.14,0.071h-0.035l-0.317,0.141v0.035l-0.141,0.071l-0.67,0.281l-0.177,0.071 l-0.035,0.035l-0.14,0.069l-0.67,0.283l-0.175,0.07l-0.035,0.035l-0.107,0.035c-6.168,2.68-12.761,4.441-19.669,5.217h-0.035 l-0.283,0.035h-0.105l-0.388,0.035h-0.107l0,0l-0.105,0.035h-0.105h-0.105l0,0l-0.212,0.035h-0.07h-0.035h-0.105l-0.107,0.036 h-0.07h-0.035h-0.105h-0.07l-0.388,0.035h-0.037h-0.105l-0.105,0.035h-0.07h-0.035h-0.107h-0.105h-0.035l-0.07,0.035h-0.105 h-0.107h-0.035h-0.07h-0.105h-0.105h-0.037l-0.07,0.035h-0.105h-0.105l0,0h-0.107h-0.105h-0.105l0,0l-0.105,0.035h-0.107h-0.105 l0,0h-0.105h-0.105h-0.072h-0.035h-0.105h-0.105h-0.07l-0.037,0.036h-0.105h-0.105h-0.07h-0.035h-0.107h-0.105h-0.035h-0.07 h-0.105h-0.107h-0.035h-0.07h-0.105h-0.105h-0.037h-0.07h-0.105l-0.105,0.035l0,0h-0.107h-0.105h-0.105l0,0h-0.105h-0.107h-0.105 l0,0h-0.105h-0.105h-0.107h-0.316h-0.282h-0.283h-0.035l-0.317-0.035h-0.281h-0.248h-0.07h-0.317l-0.318-0.036h-0.21h-0.072 h-0.316l-0.318-0.035h-0.177h-0.105l-0.318-0.035h-0.317h-0.141l-0.14-0.035h-0.318l-0.6-0.035l-0.316-0.035l-0.317-0.036 c-8.532-0.739-16.604-3.066-23.972-6.627v63.101L105.316,261.657z M127.031,142.33c-4.759-4.723-11.315-7.684-18.578-7.684 c-7.226,0-13.819,2.961-18.542,7.684c-4.759,4.76-7.719,11.316-7.719,18.578s2.96,13.818,7.719,18.578 c4.724,4.758,11.316,7.685,18.542,7.685c7.263,0,13.819-2.927,18.578-7.685c4.759-4.76,7.686-11.316,7.686-18.578 S131.79,147.09,127.031,142.33z M218.51,52.898c-2.397,0-4.336-1.939-4.336-4.336c0-2.397,1.939-4.336,4.336-4.336h39.199 c2.362,0,4.336,1.938,4.336,4.336c0,2.396-1.974,4.336-4.336,4.336H218.51z M209.52,130.98c-2.397,0-4.336-1.939-4.336-4.336 c0-2.397,1.939-4.336,4.336-4.336h116.752c2.397,0,4.336,1.938,4.336,4.336c0,2.396-1.938,4.336-4.336,4.336H209.52z M209.52,156.996c-2.397,0-4.336-1.939-4.336-4.336c0-2.361,1.939-4.336,4.336-4.336h116.752c2.397,0,4.336,1.975,4.336,4.336 c0,2.396-1.938,4.336-4.336,4.336H209.52z M184.209,227.852c-2.397,0-4.336-1.939-4.336-4.336c0-2.362,1.939-4.301,4.336-4.301 h142.063c2.397,0,4.336,1.938,4.336,4.301c0,2.396-1.938,4.336-4.336,4.336H184.209z M184.209,255.101 c-2.397,0-4.336-1.938-4.336-4.336c0-2.362,1.939-4.301,4.336-4.301h142.063c2.397,0,4.336,1.938,4.336,4.301 c0,2.397-1.938,4.336-4.336,4.336H184.209z M184.209,282.35c-2.397,0-4.336-1.938-4.336-4.336c0-2.361,1.939-4.336,4.336-4.336 h142.063c2.397,0,4.336,1.975,4.336,4.336c0,2.397-1.938,4.336-4.336,4.336H184.209z M184.209,309.6 c-2.397,0-4.336-1.939-4.336-4.336c0-2.361,1.939-4.336,4.336-4.336h142.063c2.397,0,4.336,1.975,4.336,4.336 c0,2.396-1.938,4.336-4.336,4.336H184.209z M118.148,466.223c-2.397,0-4.336-1.939-4.336-4.336c0-2.397,1.939-4.336,4.336-4.336 h225.503c2.397,0,4.336,1.938,4.336,4.336c0,2.396-1.938,4.336-4.336,4.336H118.148z M233.174,485.999 c-2.397,0-4.336-1.939-4.336-4.337c0-2.361,1.939-4.336,4.336-4.336h184.471c2.397,0,4.336,1.975,4.336,4.336 c0,2.397-1.938,4.337-4.336,4.337H233.174z M436.539,485.999c-2.396,0-4.336-1.939-4.336-4.337c0-2.361,1.939-4.336,4.336-4.336 h23.056c2.397,0,4.336,1.975,4.336,4.336c0,2.397-1.938,4.337-4.336,4.337H436.539z M66.575,485.399 c-2.397,0-4.336-1.939-4.336-4.336c0-2.362,1.939-4.336,4.336-4.336H154c2.362,0,4.336,1.974,4.336,4.336 c0,2.396-1.974,4.336-4.336,4.336H66.575z M108.454,125.975c9.66,0,18.402,3.912,24.712,10.258 c6.31,6.31,10.223,15.053,10.223,24.676c0,9.659-3.913,18.401-10.223,24.711c-6.31,6.311-15.052,10.224-24.712,10.224 c-9.623,0-18.365-3.913-24.675-10.224c-6.31-6.31-10.223-15.052-10.223-24.711c0-9.623,3.913-18.366,10.223-24.676 C90.088,129.887,98.831,125.975,108.454,125.975z"></path>{" "}
                         </g>{" "}
                    </g>{" "}
                    <g id="Layer_1"></g>{" "}
               </g>
          </svg>
     );
};
export const ProjectManagementIcon = () => {
     return (
          <svg
               viewBox="0 0 24 24"
               fill="none"
               xmlns="http://www.w3.org/2000/svg"
          >
               <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
               <g
                    id="SVGRepo_tracerCarrier"
                    strokeLinecap="round"
                    strokeLinejoin="round"
               ></g>
               <g id="SVGRepo_iconCarrier">
                    {" "}
                    <path
                         d="M11 21H4C4 17.4735 6.60771 14.5561 10 14.0709M19.8726 15.2038C19.8044 15.2079 19.7357 15.21 19.6667 15.21C18.6422 15.21 17.7077 14.7524 17 14C16.2923 14.7524 15.3578 15.2099 14.3333 15.2099C14.2643 15.2099 14.1956 15.2078 14.1274 15.2037C14.0442 15.5853 14 15.9855 14 16.3979C14 18.6121 15.2748 20.4725 17 21C18.7252 20.4725 20 18.6121 20 16.3979C20 15.9855 19.9558 15.5853 19.8726 15.2038ZM15 7C15 9.20914 13.2091 11 11 11C8.79086 11 7 9.20914 7 7C7 4.79086 8.79086 3 11 3C13.2091 3 15 4.79086 15 7Z"
                         stroke="#000000"
                         strokeWidth="2"
                         strokeLinecap="round"
                         strokeLinejoin="round"
                    ></path>{" "}
               </g>
          </svg>
     );
};
export const SendIcon = () => {
     return (
          <svg
               xmlns="http://www.w3.org/2000/svg"
               width="24"
               height="24"
               viewBox="0 0 24 24"
               fill="none"
               stroke="currentColor"
               strokeWidth="2"
               strokeLinecap="round"
               strokeLinejoin="round"
          >
               <path d="m22 2-7 20-4-9-9-4Z" />
               <path d="M22 2 11 13" />
          </svg>
     );
};
export const XIcon = () => {
     return (
          <svg
               xmlns="http://www.w3.org/2000/svg"
               width="24"
               height="24"
               viewBox="0 0 24 24"
               fill="none"
               stroke="currentColor"
               strokeWidth="2"
               strokeLinecap="round"
               strokeLinejoin="round"
          >
               <path d="M18 6 6 18" />
               <path d="m6 6 12 12" />
          </svg>
     );
};
