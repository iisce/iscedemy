  ...
  export async function splitPayment(amount: number, tutorId: string, transactionId: string) {
  ...
  throw new Error("Tutor subaccount  or bank details not found");
    ...
    const tutorSharePercentage = 0.40; // 40%
    ...
    const paystackCharge = grossTutorAmount * paystackChargePercentage + flatFee;
    const netTutorAmount = grossTutorAmount - paystackCharge;
    const companyAmount = amount - grossTutorAmount;

    if (netTutorAmount < 0) throw new Error("Insufficient amount for tutor after charges");
    ...
    if (!balanceResponse.ok || balanceData.data.available_balance < netTutorAmount * 100) {
    ...
    // Create or retrieve transfer recipient
      ...
      if(existingRecipientData.status && existingRecipientData.data.length > 0) {
        ...
        if (!recipientResponse.ok) {
          ...
          return { error: recipientData.message || "Failed to create recipient" };
        }
    ...
    if (!transferResponse.ok) {
      ...
      return { error: transferData.message || "Failed to transfer to tutor" };
    }
    ...
    console.log(`Tutor received: ₦${netTutorAmount.toFixed(2)}, Company: ₦${companyAmount.toFixed(2)}`);

    revalidatePath("/tutor/profile");
    return { success: `Payment split completed. Tutor: ₦${netTutorAmount.toFixed(2)}, Company: ₦${companyAmount.toFixed(2)}` };
  }